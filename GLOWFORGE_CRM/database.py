...
logger = logging.getLogger("glowforge.database")
...
def init_db():
    ...
    conn.executescript("""
        ...
        -- V1 Stabilization Patch: schema_hash column added inline above
    ...
    # ====== Revenue Closure Layer: 财务终态 ======
    try:
        conn.execute("ALTER TABLE orders ADD COLUMN revenue_status TEXT DEFAULT 'unclosed'")
    except:
        pass
    try:
    ...
    # ====== V1.6 D/E: 资金流程追踪 ======
    try:
        conn.execute("ALTER TABLE orders ADD COLUMN deposit_status TEXT DEFAULT ''")
    except:
        pass
    ...
    # V1 Stabilization Patch: quotes.schema_hash (safe for existing DBs)
    try:
        conn.execute("ALTER TABLE quotes ADD COLUMN schema_hash TEXT DEFAULT ''")
    except Exception:
        pass
    ...
    # ==================== V6 — Business Graph tables ====================
    init_graph_tables(conn)

    # ==================== V7 — Constraint Governance tables ====================
    init_v7_tables(conn)

    # ==================== V8 — Execution Intelligence tables ====================
    init_v8_tables(conn)

    # ==================== V9 — Autonomous Business Organization tables ====================
    init_v9_tables(conn)

    # ==================== V1.7 — Escalation Routing Engine ====================
    init_escalation_tables(conn)

    # ==================== UEE — Unified Escalation Engine ====================
    init_uee_tables(conn)

    # ==================== V8.5-B — Constraint Causal Memory ====================
    init_v85b_tables(conn)

    # ==================== V8 Kernel Migrations ====================
    # execution_key 列（idempotency guard）
    try:
        conn.execute("ALTER TABLE execution_queue ADD COLUMN execution_key TEXT DEFAULT ''")
    except Exception:
        pass
    try:
        conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_execution_key ON execution_queue(execution_key)"
            " WHERE execution_key != ''"
        )
    except Exception:
        pass
    ...
    try:
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_dlq_key ON execution_dlq(execution_key)"
        )
    except Exception:
        pass

    # ==================== V8.2 — Execution Consistency Layer ====================
    # trace_log 列（执行追踪日志）
    try:
        conn.execute("ALTER TABLE execution_queue ADD COLUMN trace_log TEXT DEFAULT '[]'")
        logger.info("[V8.2] Added trace_log column to execution_queue")
    except Exception:
        pass

    # ==================== V8.3 — Business Policy Execution Gate ====================
    # hold_reason 列（HOLD_FOR_REVIEW 原因）
    try:
        conn.execute("ALTER TABLE execution_queue ADD COLUMN hold_reason TEXT DEFAULT ''")
        logger.info("[V8.3] Added hold_reason column to execution_queue")
    except Exception:
        pass

    # V8.3: 更新 CHECK 约束支持 'hold' 状态
    try:
        row = conn.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='execution_queue'"
        ).fetchone()
        if row and "'hold'" not in (row[0] or ""):
            logger.info("[V8.3] Updating execution_queue CHECK constraint to include 'hold'")
            conn.executescript("""
                CREATE TABLE execution_queue_v83_tmp (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    task_type TEXT NOT NULL,
                    payload_json TEXT DEFAULT '{}',
                    priority INTEGER DEFAULT 5,
                    status TEXT DEFAULT 'pending' CHECK(status IN ('pending','processing','completed','failed','blocked','cancelled','hold')),
                    retry_count INTEGER DEFAULT 0,
                    max_retries INTEGER DEFAULT 3,
                    firewall_verdict TEXT DEFAULT '',
                ...
                INSERT INTO execution_queue_v83_tmp SELECT * FROM execution_queue;
                DROP TABLE execution_queue;
                ALTER TABLE execution_queue_v83_tmp RENAME TO execution_queue;
            """)
            logger.info("[V8.3] execution_queue CHECK constraint updated")
    except Exception as e:
        logger.warning("[V8.3] CHECK constraint update failed (may already be updated): %s", e)

    # 种子业务执行策略
    seed_business_execution_policies(conn)

    conn.commit()
    conn.close()


def init_graph_tables(conn):
    """初始化 Business Graph 表（graph_nodes, graph_edges, profit_paths）"""
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS profit_paths (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE INDEX IF NOT EXISTS idx_g_nodes_type ON graph_nodes(node_type);
        CREATE INDEX IF NOT EXISTS idx_g_edges_from ON graph_edges(from_node);
        CREATE INDEX IF NOT EXISTS idx_g_edges_to ON graph_edges(to_node);
        CREATE INDEX IF NOT EXISTS idx_g_edges_type ON graph_edges(edge_type);
    """)
    logger.info("Business Graph tables initialized")




def init_v7_tables(conn):
    """Initialize V7 Constraint Governance tables"""
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS decision_log (
            ...
            verdict TEXT NOT NULL CHECK(verdict IN ("ALLOW","BLOCK","MODIFY","FLAG","ESCALATE")),
        ...
        CREATE TABLE IF NOT EXISTS risk_events (
            ...
            customer_id INTEGER,
            action_type TEXT NOT NULL,
            dim_customer_value REAL DEFAULT 0.0,
            dim_amount REAL DEFAULT 0.0,
            dim_country REAL DEFAULT 0.0,
            dim_language REAL DEFAULT 0.0,
            dim_profit REAL DEFAULT 0.0,
            dim_compliance REAL DEFAULT 0.0,
        ...
        CREATE INDEX IF NOT EXISTS idx_policies_active ON policies(is_active, priority);
        CREATE INDEX IF NOT EXISTS idx_policies_check_type ON policies(check_type);
        ...
        CREATE INDEX IF NOT EXISTS idx_risk_events_score ON risk_events(overall_score);
        CREATE INDEX IF NOT EXISTS idx_firewall_events_check ON firewall_events(check_name);
        CREATE INDEX IF NOT EXISTS idx_firewall_events_customer ON firewall_events(customer_id);
    """)
    logger.info("V7 Constraint Governance tables initialized")


def init_v8_tables(conn):
    """Initialize V8 Execution Intelligence Layer tables"""
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS execution_tasks (
            ...
            task_type TEXT NOT NULL,
            status TEXT DEFAULT 'pending' CHECK(status IN ('pending','processing','completed','failed','blocked','cancelled')),
            priority INTEGER DEFAULT 5,
        ...
        CREATE INDEX IF NOT EXISTS idx_queue_locked ON execution_queue(locked_at);
        CREATE INDEX IF NOT EXISTS idx_tasks_goal ON execution_tasks(goal_id);
        CREATE INDEX IF NOT EXISTS idx_tasks_status ON execution_tasks(status);
        CREATE INDEX IF NOT EXISTS idx_agents_type ON execution_agents(agent_type);
        CREATE INDEX IF NOT EXISTS idx_feedback_goal ON execution_feedback(goal_id);
        CREATE INDEX IF NOT EXISTS idx_feedback_customer ON execution_feedback(customer_id);
    """)
    logger.info("V8 Execution Intelligence tables initialized")


# ==================== V8.5-B — Constraint Causal Memory ====================

def init_v85b_tables(conn):
    """Initialize V8.5-B Constraint Causal Memory Layer tables."""
    conn.execute("""
        CREATE TABLE IF NOT EXISTS constraint_snapshots (
            id                    INTEGER PRIMARY KEY AUTOINCREMENT,
            task_id               INTEGER REFERENCES execution_queue(id),
            ...
            allows_json           TEXT NOT NULL DEFAULT '[]',
    ...
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS constraint_execution_bindings (
            id                INTEGER PRIMARY KEY AUTOINCREMENT,
            snapshot_id       INTEGER NOT NULL REFERENCES constraint_snapshots(id),
            task_id           INTEGER NOT NULL REFERENCES execution_queue(id),
            binding_type      TEXT NOT NULL CHECK(binding_type IN ('pre_execution','post_execution','re_evaluation')),
            execution_outcome TEXT NOT NULL CHECK(execution_outcome IN ('allowed','blocked','held','completed','failed')),
            created_at        REAL NOT NULL
        )
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_task     ON constraint_snapshots(task_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_customer ON constraint_snapshots(customer_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_created  ON constraint_snapshots(created_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bindings_snapshot  ON constraint_execution_bindings(snapshot_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bindings_task      ON constraint_execution_bindings(task_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_bindings_type      ON constraint_execution_bindings(binding_type)")
    # V8.5-C query engine indexes
    conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_verdict  ON constraint_snapshots(verdict)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_task_type ON constraint_snapshots(task_type)")
    logger.info("V8.5-B Constraint Causal Memory tables initialized")
...
def add_quote(data):
    ...
    conn.execute(
        "INSERT INTO quotes (quote_no,customer_id,items,total_amount,currency,status,valid_until,notes,created_by,schema_hash) VALUES (?,?,?,?,?,?,?,?,?,?)",
        (
            quote_no,
...
def update_quote(qid, data):
    ...
    sets = []
    vals = []
    for k in ("customer_id", "total_amount", "currency", "status", "valid_until", "notes", "pdf_path", "schema_hash"):
        if k in data:
            sets.append(f"{k}=?")
            vals.append(data[k])
    if "items" in data:
...
def get_order(oid):
    ...
    row = conn.execute(
        """SELECT o.*, c.name as customer_name, c.company as customer_company,
           u.display_name as assigned_name,
           p.name as partner_name,
           COALESCE((SELECT SUM(amount) FROM payments WHERE order_id=o.id), 0) as paid_amount_calculated
           FROM orders o LEFT JOIN customers c ON o.customer_id=c.id
           LEFT JOIN users u ON o.assigned_to=u.id
           LEFT JOIN partners p ON o.partner_id=p.id
           WHERE o.id=?""",
    ...
    d["profit"] = round(revenue - cost, 2)
    d["profit_margin"] = round(d["profit"] / revenue * 100, 1) if revenue else 0
    # Revenue Closure: 计算已付金额 + 评估财务终态
    d["paid_amount_calculated"] = float(d.get("paid_amount_calculated", 0) or 0)
    if not d.get("revenue_status"):
        d["revenue_status"] = "unclosed"
    return d


def evaluate_revenue_closure(order):
    """评估订单的财务终态

    核心规则:
      - status != "delivered" → "unclosed"（物流未完成）
      - delivered + 已付 < 总额 → "partially_closed"（已交货但尾款未清）
      - delivered + 已付 >= 总额 → "closed"（收入确认完成）

    Args:
        order: get_order() 返回的 dict（必须含 paid_amount_calculated）
    Returns:
        "unclosed" | "partially_closed" | "closed"
    """
    if order.get("status") != "delivered":
        return "unclosed"
    paid = float(order.get("paid_amount_calculated", 0) or 0)
    # 回退: paid_amount_calculated 来自 payments 表子查询
    # 如果子查询无数据但 paid_amount 列有值, 用列值
    if paid <= 0:
        paid = float(order.get("paid_amount", 0) or 0)
    # 最后回退: 从 deposit/balance 字段推导（兼容老数据）
    if paid <= 0:
        dep = float(order.get("deposit_amount", 0) or 0)
        bal = float(order.get("balance_amount", 0) or 0)
        if order.get("deposit_received"):
            paid += dep
        if order.get("balance_received"):
            paid += bal
...
def add_order(data):
    ...
    conn.execute(
        ...
        (
            ...
            data.get("revenue_status", "unclosed"),
...
def get_production_schedule():
    ...
    rows = conn.execute("""
        ...
        LEFT JOIN customers c ON o.customer_id=c.id
        LEFT JOIN users u ON o.assigned_to=u.id
        WHERE o.status IN ('active','in_production','shipped')
        ORDER BY
          CASE o.status
    ...
    result = {"active": [], "in_production": [], "shipped": []}
    for r in rows:
        item = {
            "id": r["id"],
            "order_no": r["order_no"],
...
def init_v9_tables(conn):
    """Initialize V9 Autonomous Business Organization tables"""
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS org_decisions (
            ...
            status TEXT DEFAULT 'approved' CHECK(status IN ('pending','approved','rejected','executed')),
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
        CREATE TABLE IF NOT EXISTS org_departments (
            ...
            name TEXT NOT NULL,
            head_agent TEXT DEFAULT '',
            budget_pct REAL DEFAULT 0.0,
        ...
        CREATE TABLE IF NOT EXISTS org_budget_allocations (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            period TEXT NOT NULL,
            dept_id TEXT NOT NULL,
            amount REAL DEFAULT 0.0,
            pct REAL DEFAULT 0.0,
            previous_pct REAL DEFAULT 0.0,
            reason TEXT DEFAULT '',
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
        CREATE INDEX IF NOT EXISTS idx_org_decisions_session ON org_decisions(session_id);
        CREATE INDEX IF NOT EXISTS idx_org_decisions_type ON org_decisions(decision_type);
        CREATE INDEX IF NOT EXISTS idx_org_decisions_created ON org_decisions(created_at);
        CREATE INDEX IF NOT EXISTS idx_org_allocations_period ON org_budget_allocations(period);
    """)
    logger.info("V9 Autonomous Business Organization tables initialized")


def init_escalation_tables(conn):
    """Initialize V1.7 Escalation Routing Engine tables"""
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS escalation_events (
            ...
            category TEXT DEFAULT 'GENERAL'
                CHECK(category IN ('PRICE_OBJECTION','TECH_SUPPORT','COMPLAINT','GENERAL')),
            route_group TEXT DEFAULT 'sales_team'
                CHECK(route_group IN ('sales_team','engineer_team','ops_manager')),
            assigned_to INTEGER DEFAULT NULL,
            status TEXT DEFAULT 'open'
                CHECK(status IN ('open','acknowledged','resolved','closed','escalated')),
            escalation_level INTEGER DEFAULT 0,
            sla_seconds INTEGER DEFAULT 86400,
        ...
        CREATE INDEX IF NOT EXISTS idx_escalation_status ON escalation_events(status);
        CREATE INDEX IF NOT EXISTS idx_escalation_customer ON escalation_events(customer_id);
        CREATE INDEX IF NOT EXISTS idx_escalation_sla ON escalation_events(status, sla_due_at);
        CREATE INDEX IF NOT EXISTS idx_escalation_route ON escalation_events(route_group, status);
    """)
    # Migration: add columns if not exist (for existing DBs)
    for col_sql in [
        "ALTER TABLE escalation_events ADD COLUMN category TEXT DEFAULT 'GENERAL'",
        "ALTER TABLE escalation_events ADD COLUMN route_group TEXT DEFAULT 'sales_team'",
        "ALTER TABLE escalation_events ADD COLUMN sla_due_at TEXT DEFAULT ''",
        "ALTER TABLE escalation_events ADD COLUMN sla_seconds INTEGER DEFAULT 86400",
        "ALTER TABLE escalation_events ADD COLUMN assigned_to INTEGER DEFAULT NULL",
        "ALTER TABLE escalation_events ADD COLUMN resolved_at TEXT DEFAULT ''",
        "ALTER TABLE escalation_events ADD COLUMN resolved_by INTEGER DEFAULT NULL",
        "ALTER TABLE escalation_events ADD COLUMN notified_at TEXT DEFAULT ''",
        "ALTER TABLE escalation_events ADD COLUMN escalation_level INTEGER DEFAULT 0",
    ]:
        try:
            conn.execute(col_sql)
        except Exception:
            pass
    logger.info("V1.7 Escalation Routing Engine tables initialized")


# ==================== UEE — Unified Escalation Engine tables ====================

def init_uee_tables(conn=None):
    """初始化 Unified Escalation Engine 表

    UEE 是 System Control Plane 的核心队列，汇总 4 类 escalation：
    PAYMENT / EXECUTION / SLA / SYSTEM
    """
    if conn is None:
        conn = get_db()
        close_after = True
    ...
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS unified_escalations (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            severity_label TEXT DEFAULT 'medium',
            status TEXT DEFAULT 'open'
                CHECK(status IN ('open','assigned','in_progress','resolved','escalated','dismissed')),
            owner TEXT DEFAULT '',
        ...
        CREATE INDEX IF NOT EXISTS idx_uee_type ON unified_escalations(escalation_type, status);
        CREATE INDEX IF NOT EXISTS idx_uee_severity ON unified_escalations(severity, status);
        CREATE INDEX IF NOT EXISTS idx_uee_status ON unified_escalations(status);
        CREATE INDEX IF NOT EXISTS idx_uee_entity ON unified_escalations(entity_type, entity_id);
        CREATE INDEX IF NOT EXISTS idx_uee_customer ON unified_escalations(customer_id);
        CREATE INDEX IF NOT EXISTS idx_uee_sla ON unified_escalations(status, sla_deadline);
    """)

    logger.info("UEE tables initialized")
...
def add_unified_escalation(event):
    """插入一条 UEE 事件（先检查是否已存在未解决的相同 source）"""
    conn = get_db()
    existing = conn.execute(
        """SELECT id FROM unified_escalations
           WHERE source_id=? AND source_table=? AND escalation_source=?
             AND status NOT IN ('resolved','dismissed')
           LIMIT 1""",
        (event.get("source_id"), event.get("source_table", ""), event.get("escalation_source", ""))
    ).fetchone()
    if existing:
        conn.close()
        return existing[0]  # 已存在，返回 ID

    import json
    conn.execute(
        """INSERT INTO unified_escalations
           (source_id, source_table, escalation_type, escalation_source,
            entity_type, entity_id, customer_id, title, description,
            severity, severity_label, status, owner, sla_deadline,
            age_minutes, context_json, action_history, escalation_level)
           VALUES (?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)""",
        (
            event.get("source_id"), event.get("source_table", ""),
            event["escalation_type"], event["escalation_source"],
            event.get("entity_type", "order"), event.get("entity_id", 0),
            event.get("customer_id"), event.get("title", ""),
            event.get("description", ""),
            event.get("severity", 0.5), event.get("severity_label", "medium"),
            event.get("status", "open"), event.get("owner", ""),
            event.get("sla_deadline", ""),
            event.get("age_minutes", 0),
            json.dumps(event.get("context", {}), ensure_ascii=False),
            json.dumps(event.get("action_history", []), ensure_ascii=False),
            event.get("escalation_level", 0),
        )
    )
    conn.commit()
...
def get_unified_escalations(escalation_type=None, status=None, severity_min=None, limit=50):
    """查询 UEE 队列"""
    conn = get_db()
    conditions = []
    params = []
    ...
    rows = conn.execute(
        f"SELECT * FROM unified_escalations WHERE {where} ORDER BY severity DESC, created_at ASC LIMIT ?",
        (*params, limit)
    ).fetchall()
    conn.close()
    import json
    ...
    for r in rows:
        ...
        try:
            ...
            d["context"] = {}
        try:
            d["action_history"] = json.loads(d.get("action_history", "[]"))
        except Exception:
            d["action_history"] = []
        result.append(d)
    return result
...
def action_unified_escalation(eid, action_type, owner, note=""):
    """对 UEE 事件执行操作：resolve / escalate / assign / dismiss"""
    from datetime import datetime
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = get_db()

    # 读取当前
    row = conn.execute("SELECT * FROM unified_escalations WHERE id=?", (eid,)).fetchone()
    if not row:
        conn.close()
        return False
    ...
    history = []
    try:
        history = json.loads(ev.get("action_history", "[]"))
    except Exception:
        pass

    if action_type == "resolve":
        conn.execute(
            "UPDATE unified_escalations SET status='resolved', owner=?, resolved_at=?, resolved_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (owner, now, owner, eid)
        )
        history.append({"action": "resolve", "owner": owner, "note": note, "timestamp": now})

    elif action_type == "escalate":
        new_level = (ev.get("escalation_level", 0) or 0) + 1
        conn.execute(
            "UPDATE unified_escalations SET status='escalated', owner=?, escalation_level=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (owner, new_level, eid)
        )
        history.append({"action": "escalate", "owner": owner, "level": new_level, "note": note, "timestamp": now})

    elif action_type == "assign":
        conn.execute(
            "UPDATE unified_escalations SET status='assigned', owner=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (owner, eid)
        )
        history.append({"action": "assign", "owner": owner, "note": note, "timestamp": now})

    elif action_type == "dismiss":
        conn.execute(
            "UPDATE unified_escalations SET status='dismissed', owner=?, resolved_at=?, resolved_by=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
            (owner, now, owner, eid)
        )
        history.append({"action": "dismiss", "owner": owner, "note": note, "timestamp": now})

    conn.execute("UPDATE unified_escalations SET action_history=? WHERE id=?",
                 (json.dumps(history, ensure_ascii=False), eid))
    conn.commit()
    conn.close()
    return True


def get_sla_risk_escalations(threshold_minutes=60, limit=20):
    """获取 SLA 高风险事件：即将超时或已超时"""
    conn = get_db()
    rows = conn.execute(
        """SELECT ue.*, c.name as customer_name, c.whatsapp
           FROM unified_escalations ue
           LEFT JOIN customers c ON ue.customer_id=c.id
           WHERE ue.status IN ('open','assigned')
             AND ue.sla_deadline != ''
             AND ue.sla_deadline <= datetime('now','localtime', ?)
           ORDER BY ue.severity DESC, ue.sla_deadline ASC
           LIMIT ?""",
        (f'+{threshold_minutes} minutes', limit)
    ).fetchall()
    conn.close()
    import json
    result = []
...
def add_escalation_event(customer_id, order_id, original_text, category, route_group,
                         sla_seconds, assigned_to=None):
    """创建一条升级事件记录"""
    conn = get_db()
    from datetime import datetime, timedelta
    sla_due = (datetime.now() + timedelta(seconds=sla_seconds)).strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        """INSERT INTO escalation_events
           (customer_id, order_id, original_text, category, route_group,
            assigned_to, sla_seconds, sla_due_at, status)
           VALUES (?,?,?,?,?,?,?,?,'open')""",
        (customer_id, order_id, original_text, category, route_group,
         assigned_to, sla_seconds, sla_due)
    )
    conn.commit()
    eid = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.close()
...
def get_escalation_events_due():
    """获取所有已超 SLA 且未关闭的升级事件（用于自动升级线程）"""
    conn = get_db()
    rows = conn.execute(
        """SELECT e.*, c.name as customer_name, c.whatsapp
           ...
           WHERE e.status IN ('open','acknowledged')
             AND e.sla_due_at != ''
             AND e.sla_due_at <= datetime('now','localtime')
...
def resolve_escalation(eid, resolved_by, status='resolved', notes=''):
    """关闭或升级一条升级事件"""
    conn = get_db()
    from datetime import datetime
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute(
        "UPDATE escalation_events SET status=?, resolved_at=?, resolved_by=?, notes=?, updated_at=CURRENT_TIMESTAMP WHERE id=?",
        (status, now, resolved_by, notes, eid)
    )
    conn.commit()
    conn.close()


def escalate_event_next_level(eid):
    """将升级事件推到下一级 escalation chain

    升级链:
        Level 0: route_group 原始责任人
        Level 1: ops_manager（运营主管）
        Level 2: admin（广播所有管理员）

    Returns:
        dict {level, route_group, sla_seconds} | None (已达最高级)
    """
    conn = get_db()
    row = conn.execute(
        "SELECT id, escalation_level, category FROM escalation_events WHERE id=?",
        (eid,)
    ).fetchone()
    if not row:
        conn.close()
    ...
    current_level = row["escalation_level"] if row["escalation_level"] else 0
    next_level = current_level + 1

    # 升级链定义
    chain = {
        1: {"route_group": "ops_manager", "sla_seconds": 3600},   # 1h
        2: {"route_group": "sales_team",  "sla_seconds": 7200},   # 2h
    }

    if next_level not in chain:
        conn.close()
        return None  # 已达最高级

    target = chain[next_level]
    from datetime import datetime, timedelta
    sla_due = (datetime.now() + timedelta(seconds=target["sla_seconds"])).strftime("%Y-%m-%d %H:%M:%S")

    conn.execute(
        """UPDATE escalation_events
           SET escalation_level=?, route_group=?, sla_seconds=?,
               sla_due_at=?, status='open', assigned_to=NULL,
               updated_at=CURRENT_TIMESTAMP
           WHERE id=?""",
        (next_level, target["route_group"], target["sla_seconds"],
         sla_due, eid)
    )
    conn.commit()
    conn.close()
...
BUSINESS_EXECUTION_POLICIES = [
    {
        "policy_id": "BIZ_CANCEL_001",
        "category": "business_execution",
        "rule": "已投产订单不得取消",
        "severity": "hard",
        "check_type": "cancel_blocked",
        "config_json": {
            "action_domains": ["cancel_order", "modify_order"],
            "blocked_stages": ["in_production", "completed", "shipped"],
            "allow_if_deposit_not_paid": True,
        },
        "priority": 50,
    },
    {
        "policy_id": "BIZ_MODIFY_001",
        "category": "business_execution",
        "rule": "投产中订单仅允许微小修改",
        "severity": "soft",
        "check_type": "modify_blocked",
        "config_json": {
            "action_domains": ["modify_order", "modify_items"],
            "minor_changes_allowed_in_production": ["color", "font", "finish"],
        },
        "priority": 60,
    },
    {
        "policy_id": "BIZ_PRICE_001",
        "category": "business_execution",
        "rule": "同一客户72小时内不得重复改价",
        "severity": "hard",
        "check_type": "price_change_cooldown",
        "config_json": {
            "action_domains": ["change_price", "adjust_discount"],
            "cooldown_hours": 72,
            "max_changes_per_month": 3,
            "require_approval_above_pct": 15,
        },
        "priority": 100,
    },
    {
        "policy_id": "BIZ_STAGE_001",
        "category": "business_execution",
        "rule": "按生产阶段门控操作权限",
        "severity": "hard",
        "check_type": "production_stage_gate",
        "config_json": {
            "action_domains": ["*"],
            "blocked_transitions": {
                "in_production": ["cancel_order", "modify_order", "refund"],
                "completed": ["cancel_order", "modify_order"],
                "shipped": ["refund"],
            },
        },
        "priority": 30,
    ...
    {
        "policy_id": "BIZ_PAY_001",
        "category": "business_execution",
        "rule": "未收定金不得投产",
        "severity": "hard",
        "check_type": "payment_state_gate",
        "config_json": {
            "action_domains": ["start_production", "confirmed"],
            "require_full_deposit_before_production": True,
            "refund_blocked_if_shipped": True,
        },
        "priority": 40,
    },
    {
        "policy_id": "BIZ_AUTO_001",
        "category": "business_execution",
        "rule": "自动操作超出阈值需人工审核",
        "severity": "soft",
        "check_type": "auto_action_risk",
        "config_json": {
            "action_domains": ["auto_discount", "auto_reply_quote"],
            "max_discount_without_review": 10,
            "require_human_review_if_customer_risk_above": 0.7,
        },
        "priority": 150,
    },
    {
        "policy_id": "BIZ_RISK_001",
        "category": "business_execution",
        "rule": "高风险客户限制自动操作",
        "severity": "hard",
        "check_type": "customer_risk_gate",
        "config_json": {
            "action_domains": ["*"],
            "high_risk_blocked_actions": ["auto_discount", "auto_approve_order"],
            "medium_risk_hold_actions": ["cancel_order", "refund", "modify_order", "change_price"],
        },
        "priority": 20,
    },
    {
        "policy_id": "BIZ_CONTRACT_001",
        "category": "business_execution",
        "rule": "大额操作需符合合约条款",
        "severity": "soft",
        "check_type": "contract_term_gate",
        "config_json": {
            "action_domains": ["cancel_order", "refund", "modify_order"],
            "min_order_amount_for_review": 10000,
        },
        "priority": 80,
    },
]
...
def seed_business_execution_policies(conn):
    """Seed V8.3 business execution policies into policies table if not present."""
    try:
        count = 0
        for p in BUSINESS_EXECUTION_POLICIES:
            existing = conn.execute(
                "SELECT COUNT(*) FROM policies WHERE policy_id=?",
                (p["policy_id"],),
            ).fetchone()[0]
            if existing == 0:
                conn.execute(
                    """INSERT INTO policies
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, 1)""",
                    (p["policy_id"], p["category"], p["rule"], p["severity"],
                     p["check_type"], json.dumps(p["config_json"], ensure_ascii=False),
                     p["priority"]),
                )
                count += 1
        if count:
            logger.info("[V8.3] Seeded %d business execution policies", count)
        return count
    except Exception as e:
        logger.warning("[V8.3] seed_business_execution_policies failed: %s", e)
        return 0
