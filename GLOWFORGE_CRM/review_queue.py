...
def mark_auto_sent(review_id):
    ...
    with _lock:
        ...
        for entry in queue:
            if entry["id"] == review_id and entry["status"] == "pending":
                # V0-SAFETY: content gate before auto-send
                try:
                    ...
                    gate_result = validate_outbound_message(
                        entry["customer_id"],
                        entry.get("edited_en") or entry["reply_en"],
                        {"tier": "LOW"},
                    )
                    if not gate_result["passed"]:
                        entry["status"] = "blocked"
                        entry["blocked_rules"] = gate_result["blocked_rules"]
                        entry["reviewed_at"] = datetime.now().isoformat()
                        _save(queue)
                        log_outbound(
                            entry["customer_id"],
                        ...
                        logger.warning(
                            "[ReviewQueue] auto-send BLOCKED by content gate: %s",
                            review_id,
                        )
                        return {"status": "blocked", "gate_result": gate_result}
                except ImportError:
                    pass  # safety module not available
