            ...
            try:
                ...
                if send_result.get("sent"):
                    ...
                    try:
                        ...
                        send_text(send_result["text"], contact_name=cust.get("name", "") or "there")
                        _wa_sent = True
                    except ImportError:
                        pass  # 无 WhatsApp 引擎，仅记录
                    except Exception as wa_e:
                        self._log_activity(slot["action"], customer_id, "error",
                                           f"WhatsApp 发送失败: {str(wa_e)[:150]}")
                    results["sent"] += 1
                    detail = f"type={decision.get('content_type')}"
                    if not _wa_sent:
                        detail += " (仅记录，未实际发送)"
                    self._log_activity(slot["action"], customer_id, "completed", detail)
                else:
                    results["skipped"] += 1
                    self._log_activity(slot["action"], customer_id, "skipped",
...
def start_v4_scheduler_background():
    ...
    # 一次性迁移：重置 push_count（历史记录全部来自从未实际发送的假推送）
    _MIGRATION_FLAG = os.path.join(BASE_DIR, ".v4_push_reset_done")
    if not os.path.exists(_MIGRATION_FLAG):
        try:
            conn = get_db()
            conn.execute("UPDATE v4_customer_state SET push_count=0")
            conn.commit()
            conn.close()
            # 写标记，避免下次重启再次重置
