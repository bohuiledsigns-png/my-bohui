...
ALI_KEY = os.environ.get("ALI_KEY") or os.environ.get("DASHSCOPE_API_KEY")
if not ALI_KEY:
    raise RuntimeError("环境变量 ALI_KEY 或 DASHSCOPE_API_KEY 未设置 — 请检查 .env 文件")
...
# ============ 火山引擎 ============
VOLC_IMG_KEY = os.environ.get("VOLC_IMG_KEY")
if not VOLC_IMG_KEY:
    raise RuntimeError("环境变量 VOLC_IMG_KEY 未设置 — 请检查 .env 文件")
VOLC_VID_KEY = os.environ.get("VOLC_VID_KEY")
if not VOLC_VID_KEY:
    raise RuntimeError("环境变量 VOLC_VID_KEY 未设置 — 请检查 .env 文件")
...
def ask_ali(prompt, text, max_tokens=1500, timeout=60, system=None):
    ...
    for attempt in range(3):
        try:
            r = requests.post(f"{ALI_BASE}/chat/completions", headers=headers, json=payload, timeout=timeout, proxies=_no_proxy)
...
def _build_sales_strategy(text, country="", history=None):
    ...
    # ====== AI 进化：从 ai_feedback 表读取规则 ======
    evolution_blocks = ""
    try:
        from ai_evolution import get_injected_strategy_blocks
        evolution_blocks = get_injected_strategy_blocks(
            text=text, country=country, intent=intent
        )
    except Exception:
        pass
...
def analyze_customer_message(text, country="", history=None, style_samples=None, sales_name="Philip", customer_id=0):
...
"""
        ...
        if customer_id > 0:
            try:
                ...
                # 1. 确定参与竞争的Agent
                agent_ids = competition.get_active_agents(
                    customer_state=customer_state or "NEW",
                    priority_class="C",
                )
                ...
                result = competition.run_competition(
                    customer_msg=text,
                    context={"state": customer_state, "priority": "C", "country": country},
                    agent_ids=agent_ids,
                )
                ...
                winner_reply = result.get("replies", {}).get(winner_id, {})

                if winner_id:
                    injected_prompt += f"""

===== V5 Agent Competition (Winner: {winner_name}) =====
The following AI sales agent was selected as the best fit for this customer:
Agent: {winner_name} ({winner_id})
Confidence Score: {winner_score}/100
Strategy: {winner_reply.get('strategy', 'standard')}
Pricing: {winner_reply.get('pricing_mode', 'standard')}

{winner_reply.get('instruction', '')}

Generate your response in the style of this winning agent.
================================================================
"""
                    # 4. 记录竞争结果到Winner Selector
                    selector.record_result(
                        competition_id=result.get("competition_id", 0),
...
"""
        ...
        # ====== V7: 销售执行指令（预计算，注入提示词） ======
        _pre_sales_state = None
        _pre_behavior = None
        try:
            from sales_state import detect_sales_state
            from sales_executor import SalesExecutor
            _pre_sales_state = detect_sales_state(message=text)
            _executor = SalesExecutor()
            _pre_behavior = _executor.execute(
                _pre_sales_state, message=text, country=country
            )
            injected_prompt += f"""

===== 销售执行指令（严格遵守） =====
{_pre_behavior['sales_instruction']}
{_pre_behavior['risk_framing'] if _pre_behavior['requires_risk_framing'] else ''}
- 回复长度不超过 {_pre_behavior['max_words']} 个英文单词
========================================
"""
        except Exception:
            pass
        ...
        # ====== V7: 注入销售阶段状态机 ======
        try:
            from sales_state import inject_sales_state
            intent = parsed.get("intent", "")
            inject_sales_state(parsed, message=text, intent=intent)
        except Exception:
            pass
        # ======================================
        ...
        try:
            if _pre_behavior:
                from sales_executor import SalesExecutor
                # 重新执行，使用 AI 解析后的更准确 intent
                if "sales_state" in parsed:
                    _final_behavior = SalesExecutor().execute(
                        parsed["sales_state"], message=text, country=country
                    )
                    parsed["sales_execution"] = {
                        "reply_type": _final_behavior["reply_type"],
                        "quote_trigger": _final_behavior["quote_trigger"],
                        "whatsapp_action": _final_behavior["whatsapp_action"],
                        "urgency_level": _final_behavior["urgency_level"],
                        "price_anchor": _final_behavior["price_anchor"],
                        "anchor_price": _final_behavior["anchor_price"],
                        "requires_risk_framing": _final_behavior["requires_risk_framing"],
                        "customer_type": _final_behavior["customer_type"],
                    }
                    # 更新 next_action 到顶层
                    parsed["next_action"] = _final_behavior["reply_type"]
        except Exception:
            pass
        # ======================================
...
def generate_image(prompt, image_data=None):
    ...
    r = _session.post(url, headers=headers, json=payload, timeout=20)
    if r.status_code != 200:
    ...
    for _ in range(30):
        time.sleep(3)
        r2 = _session.get(status_url, headers={"Authorization": f"Bearer {ALI_KEY}"}, timeout=15)
        data = r2.json()
...
# ==================== 视频生成（通义万相 文/图生视频） ====================
# 阿里云 DashScope API，支持原生 15s+ 单次生成（无需拼接）
# 计费：720P ≈ 0.5 元/秒，1080P ≈ 0.6 元/秒
...
def generate_video(prompt, quality="720p", duration=15, aspect_ratio="9:16",
                   image_path=None, seed=-1, shot_type="multi"):
    """通义万相 文/图生视频，返回 (video_url, error)

    原生支持 5~15 秒单次生成（wan2.7 支持更长时间），无需分段拼接。

    Args:
        prompt: 英文视频描述（多镜头叙事效果更好）
        quality: "720p" / "1080p" / "pro"（pro 暂用 1080p）
        duration: 视频秒数（5~15 秒，wan2.7 支持更长）
    ...
    _SIZE_MAP = {
        ...
        "1080p": {"16:9": "1920*1080", "9:16": "1080*1920",
                  ...
                  "1:1": "1080*1080"},
        "pro":   {"16:9": "1920*1080", "9:16": "1080*1920",
                  "4:3": "1440*1080",  "3:4": "1080*1440",
                  "1:1": "1080*1080"},
    }
    size = _SIZE_MAP.get(quality, _SIZE_MAP["720p"]).get(aspect_ratio, "1280*720")
    ...
    _no_proxy = {"http": None, "https": None}
    try:
        r = requests.post(VIDEO_ENDPOINT, headers=headers, json=payload, timeout=60, proxies=_no_proxy)
        if r.status_code != 200:
    ...
    print(f"[通义万相] 任务已提交: {task_id} ({quality}, {duration}s, {size})")
    # 轮询结果（通常 1~5 分钟）
    status_url = f"https://dashscope.aliyuncs.com/api/v1/tasks/{task_id}"
    for i in range(60):
        time.sleep(10)
        try:
            r2 = requests.get(status_url, headers={"Authorization": f"Bearer {ALI_KEY}"}, timeout=15, proxies=_no_proxy)
            data = r2.json()
            status = data.get("output", {}).get("task_status", "")
            if status == "SUCCEEDED":
...
# ==================== 即梦 3.0 文/图生视频 ====================
# ⚠️  重要：即梦(智能视觉) 与 方舟(Seedance) 是两套独立的 AK/SK，不可混用！
#
# 即梦 AK/SK 在火山引擎控制台 → 密钥管理 → 新建密钥（用于智能视觉即梦）
# 方舟 AK/SK 在方舟控制台 → API Key 管理（用于 doubao-seedream/seedance）
#
# 🔑 key 说明：
#   high_aes_general_v30l_zt2i     = 720P  用户控制台 key（推荐）
#   high_aes_general_v30_zt2i      = 1080P 用户控制台 key（推荐）
#   high_aes_general_v30_pro_zt2i  = Pro   用户控制台 key（推荐）
#   jimeng_t2v_v30                 = 720P  官方文档标准 key（fallback，最大 10s）
#   jimeng_t2v_v30_1080p           = 1080P 官方文档标准 key（fallback）
#   jimeng_ti2v_v30_pro            = Pro   官方文档标准 key（fallback）
#
# ⏱ duration 参数：
#   high_aes_general* 系列：duration=3~54s + width/height（支持 15s+/30s+）
#   jimeng_t2v* 系列：   frames=121/241 + aspect_ratio（最大 10s，超长需拼接）
#
# 💰 计费（元/秒）：
#   720P=0.28元/s, 1080P=0.63元/s, Pro=1元/s
#   15s 视频 ≈ 720P:4.2元, 1080P:9.45元, Pro:15元
#   30s 视频拆 2×15s 分段生成更稳定（避免单段过长导致人物畸变）
#
# ⚡ 并发限制：每个模型最多 2 个并发任务
#
# 🔒 安全：
#   - SK 泄露即失去控制权！不要提交到 git，不要截图分享
#   - 生产环境务必用环境变量 JIMENG_AK/JIMENG_SK，不要硬编码
#   - 建议定期轮换密钥（每 3-6 个月）
#   - 失败任务已扣费的额度不退还，最多重试 1 次


_JIMENG_AK = os.environ.get("JIMENG_AK")
if not _JIMENG_AK:
    raise RuntimeError("环境变量 JIMENG_AK 未设置 — 请检查 .env 文件")
_JIMENG_SK = os.environ.get("JIMENG_SK")
if not _JIMENG_SK:
    raise RuntimeError("环境变量 JIMENG_SK 未设置 — 请检查 .env 文件")
_JIMENG_SERVICE = "cv"
_JIMENG_REGION = "cn-north-1"
_JIMENG_VERSION = "2022-08-31"
_JIMENG_HOST = "visual.volcengineapi.com"
_JIMENG_SUBMIT_ACTION = "CVSync2AsyncSubmitTask"
_JIMENG_GETRESULT_ACTION = "CVSync2AsyncGetResult"

_JIMENG_REQ_KEYS = {
    # ===== 即梦视频3.0 用户控制台 key（推荐，支持 duration 3~54s） =====
    # 需要单独申请即梦(智能视觉) AK/SK，与控制台 key 对应
    "720p": "high_aes_general_v30l_zt2i",
    "1080p": "high_aes_general_v30_zt2i",
    "pro": "high_aes_general_v30_pro_zt2i",
    # ===== 即梦官方文档标准 key（fallback，仅 frames=121/241，最大 10s） =====
    "720p_std": "jimeng_t2v_v30",
    "1080p_std": "jimeng_t2v_v30_1080p",
    "pro_std": "jimeng_ti2v_v30_pro",
}
# 使用 duration+width/height 的 key（反之用 frames+aspect_ratio）
_JIMENG_USE_DURATION = {"720p", "1080p", "pro"}

def _jimeng_sign(method, path, params, headers, body_str):
    ...
    ak = _JIMENG_AK
    sk = _JIMENG_SK
    service = _JIMENG_SERVICE
    region = _JIMENG_REGION
...
def generate_video_jimeng(prompt, quality="720p", duration=15, aspect_ratio="9:16",
                          image_path=None, seed=-1):
    """即梦 3.0 文/图生视频，返回 (video_url, error)

    自动适配 key 类型：
      - jimeng_t2v_v30* 系列 → frames + aspect_ratio（最大 10s）
      - high_aes_general* 系列 → duration + width/height（3~54s）

    Args:
        prompt: 英文视频描述
        quality: "720p" / "1080p" / "pro"
        duration: 视频秒数（jimeng_t2v 系列最大 10s，自动截断）
        aspect_ratio: "16:9" / "9:16" / "4:3" / "3:4" / "1:1" / "21:9"
        image_path: 可选参考图路径（图生视频-首帧）
        seed: 随机种子，-1=随机，相同正整数=效果一致
    Returns:
        (video_url, error)
    """
    req_key = _JIMENG_REQ_KEYS.get(quality)
    if not req_key:
        return None, f"不支持的画质: {quality}，可选 720p/1080p/pro"

    use_duration_style = quality in _JIMENG_USE_DURATION

    if use_duration_style:
        # high_aes_general* 系列：duration + width/height
        # 分辨率映射表（quality × aspect_ratio → width/height）
        _RES_MAP = {
            "720p": {"16:9": (1280, 720), "9:16": (720, 1280),
                     "4:3": (960, 720),   "3:4": (720, 960),
                     "1:1": (720, 720),   "21:9": (1680, 720)},
            "1080p": {"16:9": (1920, 1080), "9:16": (1080, 1920),
                      "4:3": (1440, 1080),  "3:4": (1080, 1440),
                      "1:1": (1080, 1080),  "21:9": (2520, 1080)},
            "pro": {"16:9": (1920, 1080), "9:16": (1080, 1920),
                    "4:3": (1440, 1080),  "3:4": (1080, 1440),
                    "1:1": (1080, 1080),  "21:9": (2520, 1080)},
        }
        # 兼容 legacy / std 后缀
        base_q = quality.replace("_legacy", "").replace("_std", "")
        res = _RES_MAP.get(base_q, _RES_MAP.get(quality, _RES_MAP["720p"]))
        w, h = res.get(aspect_ratio, (1280, 720))
        d = max(3, min(duration, 54))
        body = {
            "req_key": req_key, "prompt": prompt, "seed": seed,
            "duration": d, "width": w, "height": h,
        }
        dur_str = f"{d}s"
    else:
        # jimeng_t2v_v30* 系列（标准 key）：frames + aspect_ratio
        # frames = 24 * 秒数 + 1，最大 10s
        if duration <= 5:
            frames = 121
        elif duration <= 10:
            frames = 241
        else:
            frames = 241  # 超 10s 截断，用拼接
        body = {
            "req_key": req_key, "prompt": prompt, "seed": seed,
            "frames": frames, "aspect_ratio": aspect_ratio,
        }
        dur_str = f"{frames}f≈{duration}s"
    ...
    if image_path:
        if image_path.startswith("http://") or image_path.startswith("https://"):
            ...
            # 所有版本都尝试 base64 上传，API 不支持会返回错误
            import base64
            try:
                with open(image_path, "rb") as f:
                    b64 = base64.b64encode(f.read()).decode("utf-8")
    ...
    query = f"Action={_JIMENG_SUBMIT_ACTION}&Version={_JIMENG_VERSION}"
    url = f"https://{_JIMENG_HOST}?{query}"
    signed_headers = _jimeng_sign("POST", "/", query, {"Content-Type": "application/json"}, body_str)

    try:
        r = requests.post(url, headers=signed_headers, data=body_str, timeout=30)
        if r.status_code != 200:
            return None, f"提交失败 [{r.status_code}]: {r.text[:300]}"
        data = r.json()
        if data.get("code") != 10000:
            return None, f"提交失败: {data.get('message', r.text[:200])}"
        task_id = data["data"]["task_id"]
    except Exception as e:
        return None, f"提交请求异常: {e}"

    res_str = body.get("aspect_ratio", f'{body.get("width", "?")}x{body.get("height", "?")}')
    print(f"[Jimeng] 任务已提交: {task_id} ({quality}, {dur_str}, {res_str})")

    # 轮询结果（即梦通常30s-3min）
    query_get = f"Action={_JIMENG_GETRESULT_ACTION}&Version={_JIMENG_VERSION}"
    url_get = f"https://{_JIMENG_HOST}?{query_get}"
    for i in range(60):
        time.sleep(5)
        body_get = json.dumps({"req_key": req_key, "task_id": task_id})
        signed_get = _jimeng_sign("POST", "/", query_get, {"Content-Type": "application/json"}, body_get)
        try:
            r2 = requests.post(url_get, headers=signed_get, data=body_get, timeout=15)
            resp = r2.json()
            ...
            if status == "done":
                ...
                if video_url:
                    elapsed = (i + 1) * 5
                    print(f"[Jimeng] 生成完成 ({elapsed}s): {video_url}")
                    return video_url, None
                return None, "视频URL为空"
            elif status in ("not_found", "expired"):
                return None, f"任务异常: {status}"
            elif i % 6 == 0:
                print(f"[Jimeng] 生成中... ({i*5}s, status={status})")
        except Exception as e:
            print(f"[Jimeng] 轮询异常: {e}")

    return None, "生成超时（已等待5分钟）"


def generate_video_jimeng_spliced(prompts_and_durations, quality="720p",
                                   aspect_ratio="9:16", image_path=None, seed=-1,
                                   output_path=None):
    """即梦 3.0 多段拼接 — 生成多段视频后用 FFmpeg 拼接

    注：15s 单段直接调用 generate_video_jimeng() 即可。
    本函数适用场景：
      - 30秒+ 长视频（单段过长容易人物畸变，拆多段更稳定）
      - 多场景切换（如 场景A → 场景B → 产品特写）
      - 需要不同 prompt 控制不同段落

    Args:
        prompts_and_durations: [(prompt, seconds), ...]，每段 3~54s
        quality: "720p" / "1080p" / "pro"
        aspect_ratio: "16:9" / "9:16" / etc
        image_path: 第一段的参考图（图生视频首帧）
        seed: 随机种子
        output_path: 输出 mp4 路径，默认临时文件
    Returns:
        (output_path, error)
    """
    if not prompts_and_durations:
        return None, "prompts_and_durations 为空"

    total_dur = sum(dur for _, dur in prompts_and_durations)
    print(f"[Jimeng-Spliced] 共 {len(prompts_and_durations)} 段，总时长 {total_dur}s")
    ...
    segment_paths = []
    try:
        for i, (prompt, dur) in enumerate(prompts_and_durations):
            img = image_path if i == 0 else None
            url, err = generate_video_jimeng(
                prompt, quality=quality, duration=dur,
                aspect_ratio=aspect_ratio, image_path=img, seed=seed
            )
            if err:
                return None, f"第{i+1}段生成失败: {err}"
            # 下载到本地
            seg_path = os.path.join(temp_dir, f"seg_{uuid.uuid4().hex}.mp4")
            r = requests.get(url, timeout=60)
            if r.status_code != 200:
                return None, f"第{i+1}段下载失败"
            with open(seg_path, "wb") as f:
                f.write(r.content)
            segment_paths.append(seg_path)
            print(f"[Jimeng-Spliced] 第{i+1}段已下载: {seg_path}")

        # 3. FFmpeg 拼接（含交叉淡入淡出）
        if not output_path:
            output_path = os.path.join(temp_dir, f"jimeng_final_{uuid.uuid4().hex}.mp4")
        ...
        if len(segment_paths) == 1:
            ...
            if result.returncode != 0:
                ...
                print(f"[Jimeng-Spliced] concat 失败，尝试 re-encode: {result.stderr.decode()[:200]}")
                result = subprocess.run(
                    ["ffmpeg", "-y", "-f", "concat", "-safe", "0",
                     "-i", filelist_path,
        ...
        print(f"[Jimeng-Spliced] 拼接完成: {output_path}")
...
SIGN_PROMPT_GEN_SYSTEM = """你是一个专业的广告招牌行业AI提示词工程师。你的任务是根据客户的需求信息，生成高质量的中文提示词，用于AI生图模型（通义万相/豆包Seedream）生成招牌效果图。

客户会提供以下信息：
1. 客户行业（如：餐厅、奶茶店、美容院、服装店等）
2. 招牌文字（客户招牌上要显示的文字）
3. 参考描述（客户提供的描述或参考图说明）
4. 材质偏好（不锈钢/亚克力/炫彩等）
5. 场景类型（门头实景白天/门头实景夜晚/室内/工作室/街景）

你的输出必须严格遵循以下JSON格式（纯JSON，不要markdown代码块，不要其他文字）：
{
  "title": "提示词标题（简短描述，如：餐厅门头发光字招牌_夜间效果）",
  "prompt": "中文提示词正文，详细描述招牌要呈现的效果，包括：招牌文字内容、材质质感、灯光效果、场景氛围、拍摄角度、风格参考。200-400字之间。",
  "scene": "scene类型（storefront_night/storefront_day/indoor/studio/street）",
  "style_tags": ["写实", "商业摄影", "夜景", "发光效果"],
  "suggested_material": "建议材质（如：不锈钢烤漆+LED发光字）",
  "variants": [
    "变体1：同一个招牌的另一种效果描述",
    "变体2：第三个角度或风格描述"
  ]
...
要求：
- 提示词必须包含招牌文字内容，让AI能生成带正确文字的图像
- 描述要具体：材质质感、灯光色温、发光效果、环境氛围
- 适合AI生图模型理解，避免抽象描述
- 如果是夜景场景，强调灯光效果和氛围感
- 如果是白天场景，强调材质质感和自然光
- write in Chinese
...
# ==================== Maker Engine V1: Schema Extraction ====================

EXTRACT_SIGN_SCHEMA_SYSTEM = """You are a sign specification engineer for a LED sign factory.
Extract structured parameters from the customer's natural language design request.

Output ONLY valid JSON (no markdown, no code blocks):
{
  "text": "sign text content (main text)",
  "text_secondary": "subtitle/secondary text or empty",
  "product_type": "front|back|double|neon|rgb|metal|acrylic_display|flat",
  "material": "ss304|ss316|ss304_gold|ss304_rosegold|ss304_black|ss304_blue|ss304_purple|zn|zn_paint|acrylic|aluminum",
  "electro_color": "gold|rosegold|black|blue|purple|silver|titanium|brown|red|green",
  "letter_height_cm": 80,
  "letter_thickness_cm": 8,
  "kelvin": "6000K-6500K / Cool White|3500K-4000K / Warm White|RGB Full Color|Red only|White only|Blue only|Green only|Yellow only|Purple only|Pink only",
  "background": "night|brick|wall_white|glass|studio",
  "quantity": 1,
  "layout": "inline|stacked|custom",
  "country": ""
}
Use defaults where information is insufficient. Never leave fields null.
If the customer mentions a specific country, put it in the country field."""


def extract_sign_schema(text, country=""):
    """从客户自然语言提取结构化招牌参数
    ...
    result = ask_ali("", user_prompt, max_tokens=1000, timeout=30, system=EXTRACT_SIGN_SCHEMA_SYSTEM)
    if not result:
        return _default_sign_schema(text)
    try:
        cleaned = result.strip()
        for prefix in ("```json", "```"):
            if cleaned.startswith(prefix):
        ...
        return _normalize_sign_schema(schema)
    except (json.JSONDecodeError, Exception):
        return _default_sign_schema(text)


def _default_sign_schema(text=""):
    """提取失败时的默认值"""
    return {
        "text": text[:20] if text else "SIGN",
        "text_secondary": "",
        "product_type": "front",
        "material": "ss304",
        "electro_color": "",
...
# ==================== Schema Canonical Mappings ====================

MATERIAL_CANONICAL = {
    "ss304": "ss304", "304": "ss304", "304不锈钢": "ss304", "不锈钢304": "ss304",
    "stainless steel 304": "ss304", "304 ss": "ss304", "ss 304": "ss304",
    "steel": "ss304", "stainless": "ss304", "stainless steel": "ss304", "不锈钢": "ss304",
    "ss316": "ss316", "316": "ss316", "ss 316": "ss316", "316 ss": "ss316",
    "stainless steel 316": "ss316", "316不锈钢": "ss316",
    "ss304_gold": "ss304_gold", "gold": "ss304_gold", "gold ss304": "ss304_gold",
    "gold stainless steel": "ss304_gold", "金色不锈钢": "ss304_gold", "金色": "ss304_gold",
    "ss304_rosegold": "ss304_rosegold", "rose gold": "ss304_rosegold",
    "rosegold": "ss304_rosegold", "玫瑰金": "ss304_rosegold",
    "ss304_black": "ss304_black", "black": "ss304_black", "黑色不锈钢": "ss304_black", "黑色": "ss304_black",
    "ss304_blue": "ss304_blue", "blue": "ss304_blue", "蓝色不锈钢": "ss304_blue",
    "ss304_purple": "ss304_purple", "purple": "ss304_purple", "紫色不锈钢": "ss304_purple",
    "zn": "zn", "zinc": "zn", "锌合金": "zn",
    "zn_paint": "zn_paint", "painted zinc": "zn_paint", "烤漆锌合金": "zn_paint",
    "acrylic": "acrylic", "亚克力": "acrylic", "有机玻璃": "acrylic",
    "aluminum": "aluminum", "aluminium": "aluminum", "铝": "aluminum", "铝板": "aluminum",
}

PRODUCT_TYPE_CANONICAL = {
    "front": "front", "front lit": "front", "正面发光": "front",
    "back": "back", "back lit": "back", "背面发光": "back", "背光": "back",
    "double": "double", "double sided": "double", "双面": "double", "双面发光": "double",
    "neon": "neon", "neon sign": "neon", "flex": "neon", "软灯带": "neon",
    "rgb": "rgb", "full color": "rgb", "color": "rgb", "多彩": "rgb", "全彩": "rgb",
    "metal": "metal", "metal sign": "metal", "金属": "metal",
    "acrylic_display": "acrylic_display", "亚克力展示": "acrylic_display",
    "flat": "flat", "flat cut": "flat", "平面": "flat",
}

KELVIN_CANONICAL = {
    "cool white": "6000K-6500K / Cool White", "6000k": "6000K-6500K / Cool White",
    "6500k": "6000K-6500K / Cool White", "white": "6000K-6500K / Cool White",
    "冷白": "6000K-6500K / Cool White",
    "warm white": "3500K-4000K / Warm White", "3500k": "3500K-4000K / Warm White",
    "4000k": "3500K-4000K / Warm White", "warm": "3500K-4000K / Warm White",
    "暖白": "3500K-4000K / Warm White",
    "rgb": "RGB Full Color", "full color": "RGB Full Color",
    "color changing": "RGB Full Color", "多彩": "RGB Full Color", "全彩": "RGB Full Color",
    "red": "Red only", "red only": "Red only", "红色": "Red only",
    "blue": "Blue only", "blue only": "Blue only", "蓝色": "Blue only",
    "green": "Green only", "green only": "Green only", "绿色": "Green only",
    "yellow": "Yellow only", "yellow only": "Yellow only", "黄色": "Yellow only",
    "pink": "Pink only", "pink only": "Pink only", "粉色": "Pink only",
    "purple": "Purple only", "purple only": "Purple only", "紫色": "Purple only",
    "white led": "White only", "white only": "White only", "白光": "White only",
}

BACKGROUND_CANONICAL = {
    "night": "night", "night scene": "night", "夜景": "night",
    "brick": "brick", "brick wall": "brick", "砖墙": "brick",
    "wall_white": "wall_white", "white wall": "wall_white", "白墙": "wall_white",
    "glass": "glass", "glass wall": "glass", "玻璃": "glass",
    "studio": "studio", "white background": "studio", "工作室": "studio",
}

MATERIAL_DISPLAY = {
    "ss304": "304 Stainless Steel", "ss316": "316 Stainless Steel",
    "ss304_gold": "304 Stainless Steel (Gold Finish)",
    "ss304_rosegold": "304 Stainless Steel (Rose Gold)",
    "ss304_black": "304 Stainless Steel (Black Coated)",
    "ss304_blue": "304 Stainless Steel (Blue Coated)",
    "ss304_purple": "304 Stainless Steel (Purple Coated)",
    "zn": "Zinc Alloy", "zn_paint": "Painted Zinc Alloy",
    "acrylic": "Acrylic", "aluminum": "Aluminum",
}


def _canonicalize(value, mapping, default):
    """把输入值通过 canonical 映射表转为标准值

    Args:
        value: 输入值（可能是同义词，如 "stainless steel"）
        mapping: 映射表 dict[synonym → canonical]
        default: 无匹配时的默认值
    Returns:
        str: 标准值
    """
...
def _normalize_sign_schema(raw_schema):
    """规范化招牌参数：同义词映射 + 单位标准化 + 钳位 + 默认注入

    这是 Schema 层的唯一出口。所有下游（quote/image/display）依赖此函数保证一致性。

    Args:
        raw_schema: 从 LLM 提取的原始 schema dict
    Returns:
        dict: 完全规范化的 schema
    """
    # 1. 从默认值开始（确保所有字段存在）
    schema = _default_sign_schema(raw_schema.get("text", ""))

    # 2. 文本字段（直接覆盖）
    if raw_schema.get("text"):
        schema["text"] = str(raw_schema["text"]).strip()[:20]
    if raw_schema.get("text_secondary"):
        schema["text_secondary"] = str(raw_schema["text_secondary"]).strip()[:40]
    if raw_schema.get("electro_color"):
        schema["electro_color"] = str(raw_schema["electro_color"]).strip().lower()[:20]
    if raw_schema.get("country"):
        schema["country"] = str(raw_schema["country"]).strip()[:30]
    if raw_schema.get("layout"):
        schema["layout"] = str(raw_schema["layout"]).strip().lower()
        if schema["layout"] not in ("inline", "stacked", "custom"):
            schema["layout"] = "inline"

    # 3. Canonical 字段（同义词 → 标准值）
    schema["material"] = _canonicalize(raw_schema.get("material"), MATERIAL_CANONICAL, "ss304")
    schema["product_type"] = _canonicalize(raw_schema.get("product_type"), PRODUCT_TYPE_CANONICAL, "front")
    schema["kelvin"] = _canonicalize(raw_schema.get("kelvin"), KELVIN_CANONICAL, "6000K-6500K / Cool White")
    schema["background"] = _canonicalize(raw_schema.get("background"), BACKGROUND_CANONICAL, "night")

    # 4. 数值字段（单位检测 + 钳位）
    height = raw_schema.get("letter_height_cm")
    if height is not None:
        try:
            height = float(height)
            if height > 300:  # 超过 300 → 很可能是 mm，转 cm
                height = height / 10
            schema["letter_height_cm"] = max(10, min(300, int(height)))
        except (ValueError, TypeError):
            pass

    thickness = raw_schema.get("letter_thickness_cm")
    if thickness is not None:
        try:
            thickness = float(thickness)
            if thickness > 100:  # 很可能是 mm
                thickness = thickness / 10
            schema["letter_thickness_cm"] = max(3, min(30, int(thickness)))
        except (ValueError, TypeError):
            pass

    qty = raw_schema.get("quantity")
    if qty is not None:
        try:
            schema["quantity"] = max(1, min(100, int(float(qty))))
        except (ValueError, TypeError):
            pass
...
def sign_schema_hash(schema):
    """生成招牌参数的确定性哈希

    用于：
    - quote.schema_hash（quote ↔ schema 绑定）
    - ai_generations（image ↔ schema 绑定）
    - traceability（修改追溯）

    Args:
        schema: 规范化后的 schema dict
    Returns:
        str: 16 字符十六进制哈希
    """
    import hashlib
    # 只包含关键设计字段（排除 country/layout 等非设计参数）
    key_fields = ["text", "text_secondary", "product_type", "material",
                  "electro_color", "letter_height_cm", "letter_thickness_cm",
                  "kelvin", "background", "quantity"]
    canonical = {k: schema.get(k, "") for k in key_fields}
    raw = json.dumps(canonical, sort_keys=True, ensure_ascii=False)
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def generate_sign_prompt(industry="", sign_text="", reference="", material="", scene="", image_data=""):
    """根据客户需求生成招牌可视化提示词，返回 (result_dict, error)

    Args:
        industry: 客户行业（餐厅/奶茶店/美容院/服装店等）
        sign_text: 招牌文字内容
        reference: 参考描述/图案说明
        material: 材质偏好（不锈钢/亚克力/炫彩等）
        scene: 场景类型（storefront_night/storefront_day/indoor/studio/street）
        image_data: 可选，上传的参考图/矢量图 base64 (data:image/...)
    Returns:
        (dict with title/prompt/variants, None) or (None, error_string)
    """
    try:
        if image_data:
            # 有图片：用多模态AI分析图片 + 文字需求，一次性生成提示词
            visual_prompt = f"""你是一个专业的广告招牌行业AI提示词工程师。客户上传了一张参考图（可能是招牌设计图、矢量图、AI文件或实物照片），同时提供了以下需求信息：

客户行业：{industry or '未指定'}
招牌文字：{sign_text or '未指定'}
参考描述：{reference or '未提供'}
材质偏好：{material or '未指定'}
场景类型：{scene or '未指定'}

请分析这张参考图，结合客户的需求信息，生成招牌效果图的AI生图提示词。

你的输出必须严格遵循以下JSON格式（纯JSON，不要markdown代码块，不要其他文字）：
{{
  "title": "提示词标题（简短描述）",
  "prompt": "中文提示词正文，详细描述招牌要呈现的效果。**如果参考图中有具体的设计元素/形状/布局/颜色，必须在提示词中保留这些特征**。包括：招牌文字内容、材质质感、灯光效果、场景氛围、拍摄角度、风格参考。200-400字之间。",
  "scene": "scene类型",
  "style_tags": ["写实", "商业摄影", "发光效果"],
  "suggested_material": "建议材质",
  "variants": ["变体1", "变体2"]
}}

要求：
- 提示词必须包含招牌文字内容
- **如果参考图中含有设计元素（形状、图案、布局），必须保留到提示词中**
- 描述要具体：材质质感、灯光色温、发光效果、环境氛围
- write in Chinese"""

            result = ask_ali_multimodal(visual_prompt, image_data, max_tokens=2000, timeout=90)
        else:
            # 纯文本：用qwen生成
            user_text = f"""## 客户需求
...
请根据以上信息生成招牌效果图的AI提示词。"""

            result = ask_ali("", user_text, max_tokens=2000, timeout=60, system=SIGN_PROMPT_GEN_SYSTEM)
        ...
        return None, f"JSON解析失败: {result[:200]}"
    except Exception as e:
        return None, str(e)


def generate_sign_prompt_and_image(industry="", sign_text="", reference="", material="", scene="", image_data=""):
    """生成招牌提示词 + 出图预览（豆包Seedream），返回 (result_dict, image_url, error)

    result_dict 包含 title/prompt/variants 等字段
    """
    parsed, err = generate_sign_prompt(industry, sign_text, reference, material, scene, image_data)
    if err:
        return None, None, err

    image_url, img_err = generate_image_volc(parsed.get("prompt", ""), size="2K")
    return parsed, image_url, img_err


def generate_sign_prompt_ali(industry="", sign_text="", reference="", material="", scene="", image_data=""):
    """生成招牌提示词 + 通义万相出图，返回 (result_dict, image_url, error)"""
    parsed, err = generate_sign_prompt(industry, sign_text, reference, material, scene, image_data)
    if err:
        return None, None, err

    image_url, img_err = generate_image(parsed.get("prompt", ""))
    return parsed, image_url, img_err


# ==================== 产品 → 客户宣传视频 ====================

def generate_storefront_promo_video(product_image_url, biz_name="", sign_text=""):
    """从产品图片生成店面宣传视频（即梦 图生视频），返回 (video_url, error)"""
    import tempfile
    try:
        text_info = f'reading "{sign_text}"' if sign_text else "with brand name"
        biz_info = f"for {biz_name}" if biz_name else ""

        scene = f"A cinematic storefront promotional video {biz_info}. A premium illuminated sign {text_info} mounted on a beautiful storefront facade. The sign glows with warm LED light, creating an inviting atmosphere."
        prompt = scene + """ The camera slowly zooms in on the storefront, then pans slightly to show the sign from a dynamic angle. Warm golden hour lighting. A pedestrian walks past and glances at the sign. Inside the store, warm cozy lights are visible through the window. The sign glows beautifully, creating an inviting atmosphere.

Cinematic 24fps look, warm amber and teal color grading, shallow depth of field, smooth camera movement, realistic lighting, reflections on wet ground if nighttime, premium commercial quality, 4K, vibrant colors. Perfect for social media marketing and customer acquisition."""

        resp = http_requests.get(product_image_url, timeout=30)
        if resp.status_code != 200:
            return None, "产品图片下载失败"
        ...
        video_url, err = generate_video_jimeng(
            prompt=prompt, quality="720p", duration=5,
            aspect_ratio="9:16", image_path=tmp.name, seed=-1
        )
...
def generate_image_volc(prompt, size="2K"):
    ...
    try:
        ...
        r = requests.post(f"{VOLC_BASE}/images/generations", headers=headers, json=payload, timeout=60, proxies=_np)
        if r.status_code != 200:
            return None, f"提交失败: {r.text[:200]}"
        data = r.json()
