...
class StrategyEngine:
    ...
    # ── V0-SAFETY: Shadow mode ──

    @staticmethod
    def run_shadow_analysis(customer_context=None):
        """影子模式 — 执行策略但不记录状态、不触发操作

        返回与 run_full_analysis 相同的结构，但所有输出标记 dry_run=True。
        结果写入 shadow_run.log 供后续对比。
        """
        logger.info("Shadow analysis started")
        try:
            from strategy_engine.shadow_runner import run_shadow
            result = run_shadow(
            ...
            logger.warning("Shadow analysis failed: %s", e)
