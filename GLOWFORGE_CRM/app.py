...
# 确保 stdout 使用 UTF-8，防止 Windows GBK 编码问题导致 emoji 打印异常
try:
...
_load_dotenv_loaded = load_dotenv(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env"))
if not _load_dotenv_loaded:
    print("[WARN] .env 文件未找到或加载失败，将尝试使用环境变量")
del _load_dotenv_loaded
import socket
import subprocess
...
from ai_engine import generate_sign_prompt, generate_sign_prompt_and_image, generate_sign_prompt_ali, generate_storefront_promo_video, extract_sign_schema
...
from lead_state_engine import update_lead_state, get_lead_state, init_customer_state, set_lead_state
...
# V0-SAFETY dashboard blueprint
try:
    from safety.dashboard import bp as safety_bp
    HAS_SAFETY_BP = True
except ImportError:
    HAS_SAFETY_BP = False
...
if not _secret_key:
    ...
    if os.path.exists(_key_file):
        ...
        try:
            ...
            pass  # 写入失败继续运行，仅本次会话有效
app.secret_key = _secret_key

# API 认证中间件：所有 /api/* 请求必须登录（白名单端点除外）
_PUBLIC_API_PATHS = {
    "/api/auth/login", "/api/auth/logout", "/api/auth/me",
    "/api/whatsapp-incoming", "/api/health", "/health",
}

@app.before_request
def _require_api_auth():
    """全局 API 认证检查（before_request 中间件）。"""
    if not request.path.startswith("/api/"):
        return  # 非 API 路由放行
    if request.path in _PUBLIC_API_PATHS:
        return
    # 安全 BP 的路径以 /api/safety 开头，也归入保护
    if 'user_id' not in session:
        return jsonify({"error": "请先登录"}), 401

# V0-SAFETY: register safety API endpoints
if HAS_SAFETY_BP:
    app.register_blueprint(safety_bp)
    import logging as _lg
    _lg.getLogger("glowforge").info("V0-SAFETY dashboard registered")
...
def _safe_join(directory, filename):
    """安全拼接路径，防止 path traversal 攻击。

    验证结果路径在 expected 目录内，否则抛出 ValueError。
    """
    # 防止空文件名、绝对路径
    ...
    if not resolved.startswith(expected + os.sep) and resolved != expected:
        # Also check with trailing separator for drive roots on Windows
...
# Commercial Reality Layer：商业履约安全层（订单追踪 + 资金状态机 + 现实扰动处理）
try:
    from commercial_reality import init_commercial_reality
    init_commercial_reality(app, DB_PATH)
    print("[CommercialReality] Layer initialized")
except Exception as e:
    print(f"[CommercialReality] Init failed: {e}")
...
# V6 Business Graph系统：初始化图引擎
try:
    from business_graph.engine import GraphEngine
    _graph_engine = GraphEngine()
    result = _graph_engine.init_graph()
    if result:
        print(f"[Graph] Engine initialized: {result.get('nodes', 0)} nodes, {result.get('edges', 0)} edges")
    else:
        print("[Graph] Engine initialized (empty result)")
except Exception as e:
    print(f"[Graph] Init failed: {e}")
    _graph_engine = None

# V7 Constraint Governance Layer: PolicyEngine + AuditLogger
try:
    from safety.policy_engine import PolicyEngine
    _policy_engine = PolicyEngine()
    seeded = _policy_engine.seed_default_policies()
    if seeded > 0:
        print(f"[V7] PolicyEngine seeded {seeded} default policies")
    else:
        active = len(_policy_engine.get_active_policies())
        print(f"[V7] PolicyEngine ready, {active} active policies")
except Exception as e:
    print(f"[V7] PolicyEngine init failed: {e}")
    _policy_engine = None

try:
    from safety.audit_logger import AuditLogger
    _audit_logger = AuditLogger()
    print("[V7] AuditLogger initialized")
except Exception as e:
    print(f"[V7] AuditLogger init failed: {e}")
    _audit_logger = None

# V8 Execution Intelligence Layer: Queue + Agents + Engine
try:
    from execution.execution_queue import ExecutionQueue
    from execution.kernel import ExecutionKernel
    from execution.execution_agents import (
        ExecutionEngine, SalesAgent, PricingAgent, CRMAgent,
    )
    ...
    _execution_kernel = ExecutionKernel(queue=_execution_queue, interval=5, max_retries=3)

    # 注册基础处理器
    def _handle_send_message(payload):
        """处理 send_message 类型任务"""
        from whatsapp_engine import send_text
        customer_name = payload.get("customer_name", "")
        content = payload.get("content", "")
        if customer_name and content:
            send_text(content, contact_name=customer_name)
            return {"sent": True, "to": customer_name}
        return {"sent": False, "error": "missing customer_name or content"}

    def _handle_update_crm(payload):
        """处理 update_crm 类型任务"""
        from database import update_order, add_timeline_entry
        ...
        if order_id and updates:
            ...
            if note:
                add_timeline_entry(order_id, updates.get("status", ""), 0, note)
            return {"updated": True, "order_id": order_id}
        return {"updated": False, "error": "missing order_id or updates"}

    _execution_kernel.register_handler("send_message", _handle_send_message)
    _execution_kernel.register_handler("update_crm", _handle_update_crm)

    seeded = _resource_scheduler.seed_default_agents()
    print(f"[V8] ResourceScheduler seeded {seeded} agents")

    # 启动 worker loop（真正的 V8 执行消费者）
    _execution_kernel.start()
    print(f"[V8] ExecutionKernel started (5s interval, 3 max retries)")
except Exception as e:
    print(f"[V8] Init failed (graceful degradation): {e}")
    _execution_kernel = None
    _execution_queue = None

# V8.3 Business Policy Execution Gate: 种子策略
try:
    from safety.business_execution_gate import BusinessPolicyGate
    _business_gate = BusinessPolicyGate()
    seeded = _business_gate.seed_default_policies()
    if seeded > 0:
        print(f"[V8.3] BusinessPolicyGate seeded {seeded} execution policies")
    else:
        print(f"[V8.3] BusinessPolicyGate ready (policies already seeded)")
    _business_gate = None  # WorkerLoop lazy-loads its own instance
except Exception as e:
    print(f"[V8.3] BusinessPolicyGate init failed: {e}")
    _business_gate = None

# V9 Autonomous Business Organization Layer: 自治组织系统
_v9_board = None
_v9_dept_system = None
_v9_budget_alloc = None
_v9_protocol = None
_v9_decision_loop = None
try:
    from autonomous_org import start_autonomous_org
    (_v9_board, _v9_dept_system, _v9_budget_alloc,
     _v9_protocol, _v9_decision_loop) = start_autonomous_org()
    print("[V9] Autonomous Business Organization objects ready (daemon loop disabled)")
except Exception as e:
    print(f"[V9] Init failed (graceful degradation): {e}")


# ==================== V1.7 SLA Auto-Escalation Daemon ====================
_ESCALATION_DAEMON_RUNNING = False

def _start_sla_monitor():
    """守护线程：每 60 秒检查超时 escalation，自动升级"""
    global _ESCALATION_DAEMON_RUNNING
    _ESCALATION_DAEMON_RUNNING = True
    print("[SLA Monitor] Started (check interval: 60s)")

    while _ESCALATION_DAEMON_RUNNING:
        try:
            from database import (get_escalation_events_due, resolve_escalation,
                                  add_notification, get_active_user_ids,
                                  escalate_event_next_level)
            due_events = get_escalation_events_due()
            for ev in due_events:
                customer_name = ev.get("customer_name", f"客户#{ev['customer_id']}")
                category = ev.get("category", "GENERAL")
                route = ev.get("route_group", "sales_team")
                current_level = ev.get("escalation_level", 0)

                # 尝试推到下一级
                next_level = escalate_event_next_level(ev["id"])
                if next_level:
                    level = next_level["level"]
                    new_route = next_level["route_group"]
                    sla_due_at = next_level["sla_due_at"]

                    # 通知新责任人
                    for auid in get_active_user_ids():
                        add_notification(auid, 'escalation',
                            f'🚨 SLA 升级 Lv{level} — {customer_name}',
                            f'[{category}] {route}→{new_route}, 原SLA超时, 新截止 {sla_due_at}',
                            link=f'/customers/{ev["customer_id"]}',
                            related_type='customer', related_id=ev["customer_id"])

                    print(f"[SLA Monitor] #{ev['id']} {customer_name}: "
                          f"Lv{level} {route}→{new_route}, SLA@{sla_due_at}")
                else:
                    # 已达最高级，标记 escalated 并广播
                    resolve_escalation(ev["id"], 0, "escalated",
                                       f"已达最高 escalation level ({current_level}), 广播通知")
                    for auid in get_active_user_ids():
                        add_notification(auid, 'escalation',
                            f'🚨 最高级 SLA — {customer_name}',
                            f'[{category}/{route}] Lv{current_level} 无人处理',
                            link=f'/customers/{ev["customer_id"]}',
                            related_type='customer', related_id=ev["customer_id"])
                    print(f"[SLA Monitor] #{ev['id']} {customer_name} "
                          f"→ escalated (final level {current_level})")
        except Exception as e:
            print(f"[SLA Monitor] Check error: {e}")
...
# Start SLA daemon
try:
    _sla_thread = threading.Thread(target=_start_sla_monitor, daemon=True, name="SLAMonitor")
    _sla_thread.start()
except Exception as e:
    print(f"[SLA Monitor] Failed to start: {e}")

# ====== UEE Scanner Daemon ======
try:
    from safety.unified_escalation_engine import start_uee_scanner
    _uee_thread = start_uee_scanner(interval=120)  # 每 2 分钟扫描
    print(f"[UEE Scanner] Started (interval: 120s)")
except Exception as e:
    _uee_thread = None
    print(f"[UEE Scanner] Failed to start: {e}")
...
# 允许的 prompts 子目录（白名单）
_ALLOWED_PROMPT_TYPES = {"image", "video", "script", "product"}


def _safe_prompt_path(ptype, filename):
    """安全拼接 prompts 路径：校验 ptype 白名单 + 防 traversal。"""
    if ptype not in _ALLOWED_PROMPT_TYPES:
        raise ValueError(f"不支持的 prompts 类型: {ptype}")
    return _safe_join(os.path.join(PROMPT_DIR, ptype), filename)
...
def api_prompt_content(filename):
    ptype = request.args.get("type", "image")
    try:
        path = _safe_prompt_path(ptype, filename)
    except ValueError:
...
def api_create_prompt():
    ...
    if not name.endswith(".txt"):
        name += ".txt"
    try:
        path = _safe_prompt_path(ptype, name)
    except ValueError:
        return jsonify({"error": "invalid path"}), 400
    if os.path.exists(path):
...
直接输出跟进消息，不要解释："""
    ...
    # ====== Execution Firewall: 跟进话术检查 ======
    try:
        from safety.execution_firewall import ExecutionFirewall
        from safety.state_registry import StateRegistry
        _fw_registry = StateRegistry()
        _fw = ExecutionFirewall(_fw_registry)
        _fw_action = {
            "type": "send_followup",
            "content": reply,
            "intent": "跟进",
            "source_agent": "followup_engine",
        }
        _fw_ctx = {
            "customer_country": c.get("country", "") if c else "",
            "customer_tier": c.get("lead_status", "LOW") if c else "LOW",
            "urgency": "低",
        }
        _fw_decision = _fw.check(cid, _fw_action, _fw_ctx)
        if _fw_decision["verdict"] == "BLOCK":
            print(f"[Firewall] ⛔ 跟进被拦截: {_fw_decision['reason']}")
            return jsonify({"ok": False, "error": f"防火墙拦截: {_fw_decision['reason']}"}), 403
        elif _fw_decision["verdict"] == "ESCALATE":
            print(f"[Firewall] ⚠ 跟进需审核: {_fw_decision['reason']}")
        elif _fw_decision["verdict"] == "MODIFY" and _fw_decision.get("modified_action"):
            reply = _fw_decision["modified_action"].get("content", reply)
    except ImportError:
        pass
    except Exception as e:
        print(f"[Firewall] 异常（放行）: {e}")
...
_ORDER_MSG_TEMPLATES = {
    "active": {
        "en": "Dear {name}, great news! Your deposit for order {order_no} has been confirmed. Your order is now active and will enter production soon.",
        "zh": "亲爱的{name}，好消息！订单{order_no}的定金已确认。您的订单已激活，即将进入生产环节。",
    },
    "confirmed": {
...
def api_send_whatsapp(cid):
    ...
    def job():
        try:
            # ====== Execution Firewall: 手动发送也经过策略检查 ======
            try:
                from safety.execution_firewall import ExecutionFirewall
                from safety.state_registry import StateRegistry
                _fw_registry = StateRegistry()
                ...
                _fw_ctx = {
                    "customer_country": c.get("country", "") if c else "",
                    "customer_tier": c.get("tier") or c.get("lead_status", "LOW") if c else "LOW",
                    "urgency": "中",
                    "translation": content_cn or text,
                }
                _fw_decision = _fw.check(cid, _fw_action, _fw_ctx)
                if _fw_decision["verdict"] == "BLOCK":
                    print(f"[Firewall] ⛔ 手动发送被拦截: {_fw_decision['reason']}")
                    return None
                elif _fw_decision["verdict"] == "ESCALATE":
                    print(f"[Firewall] ⚠ 手动发送需审核: {_fw_decision['reason']}")
            except ImportError:
                pass
            except Exception as e:
                print(f"[Firewall] 异常（放行）: {e}")
...
def api_whatsapp_incoming():
    """WhatsApp 服务器检测到新消息时转发到此接口（需 HMAC 签名验证）"""
    # 验证 Webhook 签名
    _webhook_secret = os.environ.get("WHATSAPP_WEBHOOK_SECRET", "")
    if _webhook_secret:
        received_sig = request.headers.get("X-Webhook-Signature", "")
        if not received_sig:
            return jsonify({"ok": False, "error": "缺少 X-Webhook-Signature 头"}), 401
        body_bytes = request.get_data()
        expected_sig = hmac.new(_webhook_secret.encode("utf-8"), body_bytes, hashlib.sha256).hexdigest()
        if not hmac.compare_digest(expected_sig, received_sig):
            return jsonify({"ok": False, "error": "无效的 webhook 签名"}), 403
    else:
        # 开发环境无 secret 时仅允许本地回环请求
        remote_ip = request.remote_addr or ""
        if remote_ip not in ("127.0.0.1", "::1", "localhost"):
            return jsonify({"ok": False, "error": "webhook secret 未配置，仅接受本地请求"}), 403

    data = request.json or {}
    chat_name = data.get("chat_name", "")
    messages = data.get("messages", [])
    if not chat_name or not messages:
        return jsonify({"ok": False, "error": "chat_name and messages required"}), 400

    # 在后台线程处理，不阻塞服务器响应
    threading.Thread(target=_whatsapp_message_handler, args=(chat_name, messages), daemon=True).start()
    return jsonify({"ok": True})
...
def _send_fallback_reply(cid, chat_name, original_text, country=""):
    """AI分析失败时发保底回复，不让客户干等"""
    try:
        fallback_en = f"Thank you for your message! I've noted it down and a team member will follow up with you shortly."
        fallback_cn = f"感谢您的消息！已记录下来，我们的同事会尽快跟进。"
        send_text(fallback_en, contact_name=chat_name)
        add_message(cid, "sent", fallback_cn, fallback_en)
        _add_wa_activity("replied", cid, chat_name, "AI分析异常，已发保底回复")
    except Exception as e:
        print(f"[Auto] 保底回复也失败了: {e}")


def _do_auto_reply(cid, chat_name, reply_en, reply_cn, bypass_queue=False):
    """在独立线程中执行自动回复，不阻塞事件循环
    - 等待1-5分钟（不定时，模拟真人）
    - 发前检查是否已被手动回复
    - bypass_queue=True: 直接发送（不经过审核队列，适用低风险回复）
    - bypass_queue=False: 加入审核队列，等待管理员批准或超时自动发
    """
    try:
        # 1-5分钟不定时，模拟真人回复节奏
        wait = random.uniform(60, 300)
        print(f"[Auto] 等待{wait:.0f}秒后{'直接发送' if bypass_queue else '审核'} {chat_name}...")
        time.sleep(wait)
        ...
        try:
            ...
            from safety.message_audit import log_outbound
            gate_result = validate_outbound_message(
                cid, reply_en, {"tier": "LOW"},
            )
            if not gate_result["passed"]:
                gate_passed = False
                print(f"[Auto] ⚠ 内容审核发现违例: {[r['rule'] for r in gate_result['blocked_rules']]}")
                log_outbound(cid, reply_en, gate_result, status="blocked")
        except ImportError:
            pass

        # 内容门未通过 → 记录日志，不发送
        if not gate_passed:
            _add_wa_activity("blocked", cid, chat_name, "内容审核拦截")
            return
        ...
        if bypass_queue:
            ...
            _add_wa_activity("replied", cid, chat_name, f"AI自动回复: {reply_en[:50]}...")
            print(f"[Auto] ✅ {chat_name} 已直接发送: {reply_en[:60]}...")
            return
...
def _try_abc_flow(text, chat_name, messages):
    """检测并处理 A/B/C 确认回复

    前置条件:
        - 客户有 status="preview" 的 quote（DESIGN_SIGN 刚发送过）
        - 回复文本匹配 A/B/C 模式
    返回:
        True = 已处理，无需继续走 AI 分析
        False = 非 A/B/C 回复，走正常流程
    """
    try:
        # 1. 找客户
        ...
        # 2. 检查是否有待确认的设计报价
        preview_quotes = get_quotes(status="preview", customer_id=c["id"])
        if not preview_quotes:
            return False

        latest_quote = preview_quotes[0]  # 最新的 preview quote

        # 3. 匹配回复模式
        t = text.strip().lower()

        # A: 确认
        if t in ("a", "a.", "yes", "confirm", "confirmed", "ok", "okay",
                 "sure", "proceed", "correct", "yeah", "yep", "good", "fine", "y", "y."):
            _handle_confirm_design(c["id"], chat_name, latest_quote)
            return True

        # B: 修改
        if t in ("b", "b.", "modify", "change", "edit", "revise") or \
           any(w in t for w in ["change", "modify", "revise", "instead", "different", "but"]):
            _handle_revise_design(c["id"], chat_name, text, latest_quote)
            return True

        # C: 不要
        if t in ("c", "c.", "no", "not", "n", "n.", "cancel", "stop", "wrong",
                 "dont", "don't", "不需要"):
            _handle_reject_design(c["id"], chat_name, latest_quote)
            return True

        return False

    except Exception as e:
        print(f"[ABC] 检测异常（放行到正常流程）: {e}")
        return False


# ==================== V1.6 D/E 资金启动 & 人工路由 ====================

# 默认银行账户信息（可在 .communication_style.json 或 DB 中覆盖）
_DEFAULT_BANK_DETAILS = {
    "bank_name": "Bank of China",
    "branch": "Shenzhen Branch",
    "account_name": "Shenzhen Bohui Sign Co., Ltd.",
    "account_no": "XXXX XXXX XXXX XXXX XXX",
    "swift": "BKCHCNBJ",
    "currency": "USD",
}

def _try_deposit_flow(text, chat_name, messages):
    """检测并处理 D/E 回复（资金启动 / 人工介入）

    前置条件:
        - 客户有 status="waiting_deposit" 的订单（V1.5 刚发送过定金请求）
        - D 回复: 客户确认付款，发送银行账户
        - E 回复: 客户有问题，转人工跟进
    返回:
        True = 已处理，无需走 AI 分析
        False = 非 D/E 回复，走正常流程
    """
    try:
        t = text.strip().lower()
        ...
        # 2. 检查是否有 waiting_deposit 的订单
        from database import get_orders, get_quotes
        pending_orders = get_orders(status="waiting_deposit", customer_id=c["id"])
        if not pending_orders:
            return False

        latest_order = pending_orders[0]

        # Guard: D/E 是 A/B/C 的下游状态，只允许有已确认报价的订单触发
        quote_id = latest_order.get("quote_id")
        if quote_id:
            parent_quote = get_quotes(id=quote_id)
            if parent_quote and parent_quote[0].get("status") not in ("confirmed", "converted"):
                print(f"[DepositFlow] Guard: {chat_name} 报价未确认，阻止 D/E")
                return False

        # 3. D：确认定金，请求银行账户
        if any(k in t for k in ["d", "d.", "send bank", "bank details", "transfer",
                                 "wire", "account", "bank", "remittance", "tt",
                                 "t/t", "pay", "payment", "deposit", "付"]):
            _handle_deposit_request(c["id"], chat_name, latest_order)
            return True

        # 4. E：有问题 / 转人工
        if any(k in t for k in ["e", "e.", "question", "help", "problem", "unsure",
                                 "confused", "doubt", "worried", "not sure",
                                 "explain", "clarify", "问", "咨询"]):
            _handle_escalation(c["id"], chat_name, text, latest_order)
            return True

        return False

    except Exception as e:
        print(f"[DepositFlow] 检测异常（放行到正常流程）: {e}")
        return False


def _handle_deposit_request(cid, chat_name, order):
    """D：客户确认定金 —> 发送银行账户 —> 更新订单状态"""
    print(f"[DepositFlow] {chat_name} 请求银行账户，order#{order['id']}")

    try:
        bank = _DEFAULT_BANK_DETAILS
        order_no = order.get("order_no", "")
        deposit_amount = float(order.get("deposit_amount", 0) or 0)
        sym = "$" if order.get("currency", "USD") == "USD" else "¥"

        # 发送银行账户信息
        msg = (
            f"Thank you for your confirmation! Here are our bank details for the deposit.\n\n"
            f"*Order Reference:* {order_no}\n"
            f"*Deposit Amount:* {sym}{deposit_amount:,.0f}\n\n"
            f"*Bank Transfer Details:*\n"
            f"Bank: {bank['bank_name']}\n"
            f"Branch: {bank['branch']}\n"
            f"Account Name: {bank['account_name']}\n"
            f"Account No: {bank['account_no']}\n"
            f"SWIFT Code: {bank['swift']}\n"
            f"Currency: {bank['currency']}\n\n"
            f"After the transfer, please send us the payment receipt screenshot. "
            f"We will confirm and start production immediately!\n\n"
            f"If you need any assistance, please feel free to ask."
        )
        send_text(msg, contact_name=chat_name)

        # 更新订单：标记银行信息已发送
        from database import update_order
        update_order(order["id"], {"deposit_status": "bank_sent"})

        # 记录活动
        _add_wa_activity("payment_info_sent", cid, chat_name,
                         f"已发送银行账户信息，订单 {order_no}，定金 {sym}{deposit_amount:,.0f}")

        print(f"[DepositFlow] {chat_name} 银行账户已发送 ✅")

    except Exception as e:
        print(f"[DepositFlow] 发送银行账户失败: {e}")
        try:
            send_text(f"Please contact our finance department for bank details. We will reply shortly.",
                      contact_name=chat_name)
        except:
            pass


# ==================== V1.7 Escalation Routing Engine ====================

# 分类器（rule-based，零 AI 开销）
_ESCALATION_CLASSIFIER_RULES = [
    ("PRICE_OBJECTION", ["price", "expensive", "discount", "cheaper", "cost",
                         "overpriced", "too much", "over budget", "afford",
                         "value", "cheap", "costly", "spend", "花钱", "贵"]),
    ("TECH_SUPPORT", ["install", "how to", "technical", "spec", "specification",
                      "dimension", "measurement", "watt", "voltage", "power",
                      "fix", "repair", "broken", "not working", "error",
                      "guide", "manual", "instruction", "wire", "connect",
                      "技术", "安装", "规格", "怎么装"]),
    ("COMPLAINT", ["delay", "late", "wrong", "damaged", "defective",
                   "missing", "not received", "complaint", "unhappy",
                   "dissatisfied", "frustrated", "angry", "terrible",
                   "poor quality", "投诉", "不满意", "损坏", "延迟"]),
]

# 路由映射
_ESCALATION_ROUTING = {
    "PRICE_OBJECTION": "sales_team",
    "TECH_SUPPORT": "engineer_team",
    "COMPLAINT": "ops_manager",
    "GENERAL": "sales_team",
}

# 路由 → 负责人 ID 硬编码（后续可改为 DB 配置）
_ESCALATION_OWNERS = {
    "sales_team": None,      # None = 通知全组
    "engineer_team": None,
    "ops_manager": None,
}
...
# SLA（秒）
_ESCALATION_SLA = {
    "PRICE_OBJECTION": 900,    # 15 min
    "TECH_SUPPORT": 3600,      # 1 hour
    "COMPLAINT": 1800,         # 30 min
...
_ESCALATION_REPLIES = {
    "PRICE_OBJECTION": (
        "I understand your concern about the pricing. "
        "I have forwarded your inquiry to our sales manager who will discuss "
        "the best options with you within 15 minutes."
    ),
    "TECH_SUPPORT": (
        "Thank you for your technical inquiry. "
        "I have transferred your question to our engineer team. "
        "They will assist you within 1 hour with detailed specifications."
    ),
    "COMPLAINT": (
        "I apologize for the inconvenience. "
        "Your issue has been escalated to our operations manager, "
        "who will follow up within 30 minutes to resolve this."
    ),
    "GENERAL": (
        "Thank you for reaching out. I have transferred your request to our "
        "sales team who will get back to you within 24 hours."
    ),
}
...
def classify_escalation(text):
    """基于规则的升级分类器，零 AI 开销"""
    t = text.lower()
    for category, keywords in _ESCALATION_CLASSIFIER_RULES:
        if any(kw in t for kw in keywords):
            return category
    return "GENERAL"


def _handle_escalation(cid, chat_name, text, order):
    """V1.7 E：分类 → 路由 → SLA → 通知责任人

    改变：
        - 旧：broadcast all admins
        - 新：分类 → 路由组 → 仅通知责任人 → SLA 计时
    """
    from database import get_active_user_ids, add_notification, add_activity_log
    from database import add_escalation_event

    print(f"[Escalation] {chat_name} 要求人工介入，order#{order['id']}")
    ...
    try:
        ...
        # 1. 分类
        category = classify_escalation(text)
        route_group = _ESCALATION_ROUTING.get(category, "sales_team")
        sla_seconds = _ESCALATION_SLA.get(category, 86400)
        label = _ESCALATION_LABELS.get(category, "📋 Inquiry")
        reply_template = _ESCALATION_REPLIES.get(category, _ESCALATION_REPLIES["GENERAL"])

        print(f"[Escalation] {chat_name}: category={category}, route={route_group}, sla={sla_seconds}s")

        # 2. 更新客户状态
        update_lead_state(cid, "NEEDS_ASSISTANCE")

        # 3. 创建 escalation_event 记录（含 SLA）
        eid = add_escalation_event(
            customer_id=cid,
            order_id=order_id,
            original_text=text,
            category=category,
        ...
        print(f"[Escalation] event#{eid} created")

        # 4. 发送分类感知的 WhatsApp 回复
        sla_minutes = sla_seconds // 60
        sla_hours = sla_seconds // 3600
        time_str = f"{sla_minutes} minutes" if sla_minutes < 120 else f"{sla_hours} hours"
        personalized_reply = reply_template.format(time=time_str)
        send_text(personalized_reply, contact_name=chat_name)

        # 5. 通知路由：仅通知该 route_group 的管理员
        user_ids = get_active_user_ids()
        # 如果有指定负责人，仅通知他
        owner_id = _ESCALATION_OWNERS.get(route_group)
        if owner_id:
            notified_users = [owner_id]
        else:
            notified_users = user_ids

        from datetime import datetime, timedelta
        sla_due = (datetime.now() + timedelta(seconds=sla_seconds)).strftime("%H:%M")
        for auid in notified_users:
            add_notification(auid, 'escalation',
                f'{label} — {chat_name}',
                f'[{route_group}] SLA {sla_minutes}min: {text[:80]}',
                link=f'/customers/{cid}',
                related_type='customer', related_id=cid)

        # 6. 记录活动
        _add_wa_activity("escalated", cid, chat_name,
                         f"[{category}]→{route_group} 订单{order_no}: {text[:60]}...")

        # 7. 记录 operation log
        add_activity_log(0, "escalate", "customer", cid,
                         f"[{category}] 客户 {chat_name} 要求人工介入 (订单 {order_no})",
                         details=text)

        print(f"[Escalation] {chat_name} → {category}/{route_group}, SLA @{sla_due} ✅")

    except Exception as e:
        print(f"[Escalation] 转人工失败: {e}")
        import traceback
        traceback.print_exc()
        try:
            send_text(f"Thank you. Your request has been noted. A team member will contact you shortly.",
                      contact_name=chat_name)
        except:
            pass


# ==================== V1.8 Payment Receipt Gate ====================

_RECEIPT_KEYWORDS = [
    "paid", "done", "transfer", "transferred", "sent", "remitted",
    "receipt", "screenshot", "payment proof", "已完成", "已付",
    "付款", "转账", "汇款", "凭证", "截图",
]

def _try_receipt_flow(text, chat_name, messages):
    """检测客户付款确认回复（F: 已付款）

    前置条件:
        - 客户有 waiting_deposit 订单且已收到银行信息 (deposit_status="bank_sent")
        - 回复文本匹配付款确认关键词
    返回:
        True = 已处理，无需走 AI 分析
    """
    try:
        ...
        # 2. 检查是否有 waiting_deposit 订单且已发送银行信息
        from database import get_orders
        pending_orders = get_orders(status="waiting_deposit", customer_id=c["id"])
        if not pending_orders:
            return False
        latest_order = pending_orders[0]
        if latest_order.get("deposit_status") != "bank_sent":
            return False

        # 3. 匹配付款确认关键词
        matched = any(kw in t for kw in _RECEIPT_KEYWORDS)
        if not matched:
            return False

        _handle_payment_confirmation(c["id"], chat_name, latest_order, text)
        return True

    except Exception as e:
        print(f"[ReceiptGate] 检测异常（放行）: {e}")
        return False


def _handle_payment_confirmation(cid, chat_name, order, original_text):
    """F: 客户确认已付款 → 确认接收 → 通知财务验证"""
    from database import update_order, add_notification, get_active_user_ids, add_activity_log

    print(f"[ReceiptGate] {chat_name} 确认已付款，order#{order['id']}")
    ...
    try:
        ...
        # 1. 更新 deposit_status
        update_order(order["id"], {"deposit_status": "payment_confirmed"})

        # 2. 发送确认回复
        msg = (
            f"Thank you for confirming the payment!\n\n"
            f"*Order:* {order_no}\n"
            f"*Amount:* {sym}{deposit_amount:,.0f}\n\n"
            f"Please share the payment receipt or screenshot "
            f"so we can verify and activate your order immediately.\n\n"
            f"Our finance team will confirm within 1-2 hours "
            f"and your production will begin right after."
        )
        send_text(msg, contact_name=chat_name)

        # 3. 通知财务 / 管理员验证
        for auid in get_active_user_ids():
            add_notification(auid, 'payment',
                f'💳 付款确认 — {chat_name}',
                f'订单 {order_no}: 客户确认已付款 {sym}{deposit_amount:,.0f}，请验证到账',
                link=f'/customers/{cid}',
                related_type='customer', related_id=cid)

        # 4. 记录活动
        _add_wa_activity("payment_confirmed", cid, chat_name,
                         f"客户确认已付款，订单 {order_no} (deposit_status→payment_confirmed)")

        add_activity_log(0, "payment_confirm", "customer", cid,
                         f"客户 {chat_name} 确认已付款 (订单 {order_no})",
                         details=original_text)

        print(f"[ReceiptGate] {chat_name} 付款确认处理完成 ✅")

    except Exception as e:
        print(f"[ReceiptGate] 处理失败: {e}")
        import traceback
        traceback.print_exc()


# ==================== V1.9 Order Status Inquiry Handler ====================

_ORDER_STATUS_KEYWORDS = [
    "where is my order", "order status", "tracking", "shipped",
    "shipment", "production", "progress", "order update", "when will",
    "how long", "any update", "ready", "finished", "done yet",
    "delivery", "arrive", "ETA", "estimated time",
    "订单状态", "进度", "生产", "发货", "什么时候好",
]

_STATUS_DISPLAY = {
    "active": "⏳ Order Confirmed — In Queue",
...
_ORDER_STATUS_REPLIES = {
    "active": (
        "Your order is confirmed and in our production queue. "
        "We will start production shortly and keep you updated."
    ),
    "in_production": (
        "Your order is currently in production. "
        "We will notify you once it's ready for shipping."
    ),
    "shipped": (
        "Your order has been shipped! "
        "Please check the shipping details below."
    ),
    "delivered": (
        "Your order has been delivered. "
        "We hope you are satisfied! Feel free to reach out if you need anything."
    ),
}

def _try_order_status_flow(text, chat_name, messages):
    """检测客户查询订单状态的回复

    前置条件:
        - 客户有 active/in_production/shipped/delivered 的订单
        - 回复文本匹配订单状态关键词
    返回:
        True = 已处理，无需走 AI 分析
    """
    try:
        ...
        # 2. 匹配关键词
        if not any(kw in t for kw in _ORDER_STATUS_KEYWORDS):
            return False

        # 3. 查找客户的活跃订单
        from database import get_orders
        trackable_statuses = ["active", "in_production", "shipped", "delivered"]
        for status in trackable_statuses:
            orders = get_orders(status=status, customer_id=c["id"], limit=1)
            if orders:
                _handle_order_status_inquiry(c["id"], chat_name, orders[0])
                return True

        return False

    except Exception as e:
        print(f"[OrderStatus] 检测异常（放行）: {e}")
        return False


def _handle_order_status_inquiry(cid, chat_name, order):
    """查询订单真实状态并返回"""
    print(f"[OrderStatus] {chat_name} 查询订单 #{order['id']}")

    try:
        order_no = order.get("order_no", "")
        status = order.get("status", "")
        progress = order.get("production_progress", 0) or 0
        start_date = order.get("production_start_date", "") or ""
        end_date = order.get("production_end_date", "") or ""
        shipping = order.get("shipping_info", "") or ""
        status_label = _STATUS_DISPLAY.get(status, status)
        reply = _ORDER_STATUS_REPLIES.get(status, "")

        lines = [
            f"*Order Status:* {status_label}",
            f"*Reference:* {order_no}",
        ]

        # 阶段特定的详细信息
        if status in ("in_production",):
            lines.append(f"*Progress:* {progress}%")
            if start_date:
                lines.append(f"*Started:* {start_date}")
            if end_date:
                lines.append(f"*Est. Completion:* {end_date}")

        elif status == "shipped":
            lines.append(f"*Progress:* 100%")
            lines.append(f"*Shipped:* ✅")
            if shipping:
                lines.append(f"*Tracking:* {shipping}")
            if end_date:
                lines.append(f"*Est. Delivery:* {end_date}")

        elif status == "active":
            lines.append(f"*Progress:* {progress}%")
            if end_date:
                lines.append(f"*Est. Start:* {end_date}")
        ...
        if reply:
            lines.insert(0, "")
            lines.insert(0, reply)

        msg = "\n".join(lines)
        send_text(msg, contact_name=chat_name)

        _add_wa_activity("order_status", cid, chat_name,
                         f"客户查询订单状态: {order_no} ({status}, {progress}%)")

        print(f"[OrderStatus] {chat_name} → {order_no} ({status}, {progress}%) ✅")

    except Exception as e:
        print(f"[OrderStatus] 查询失败: {e}")
        import traceback
        traceback.print_exc()
        try:
            send_text(f"Let me check on your order and get back to you shortly.",
                      contact_name=chat_name)
        except:
            pass


# ==================== Active Order Change Request ====================

_ORDER_CHANGE_KEYWORDS = {
    "change_text": ["change text", "change the text", "change word", "different text",
                    "modify text", "new text", "change the word", "change letter",
                    "change name", "change wording", "different wording",
                    "换个字", "改文字", "改内容", "换内容", "改字"],
    "change_material": ["change material", "change the material", "different material",
                        "switch material", "other material", "stainless instead",
                        "acrylic instead", "instead of ss", "instead of stainless",
                        "改材质", "换材质", "不锈钢改", "改不锈钢"],
    "change_size": ["change size", "change the size", "bigger", "smaller", "resize",
                    "different size", "change height", "change the height",
                    "change width", "change dimension", "make it bigger",
                    "make it smaller", "make it larger",
                    "放大", "缩小", "改尺寸", "改大小", "改高度", "大一点", "小一点"],
    "change_quantity": ["add more", "additional", "extra", "change quantity",
                        "change the quantity", "more units", "increase quantity",
                        "增加数量", "再加", "加多", "加数量"],
    "change_shipping": ["change address", "change my address", "change shipping",
                        "change the address", "change shipping address",
                        "change my shipping address", "different address",
                        "different shipping address", "new address",
                        "update address", "update my address", "update shipping address",
                        "update my shipping address", "ship to",
                        "different shipping",
                        "改地址", "换地址", "改收货", "改物流"],
    "cancel": ["cancel order", "cancel my order", "cancel the order",
               "stop order", "don't want", "i want to cancel",
               "取消订单", "不想要了", "取消"],
}

# 修改类型 → 可行性说明
_ORDER_CHANGE_FEASIBILITY = {
    "change_text": {
        "active": "flexible",
        "in_production": "limited",
        "shipped": "blocked",
    },
    "change_material": {
        "active": "flexible",
...
_ORDER_CHANGE_REPLIES = {
    "flexible": (
        "Sure! Your order is still early enough to accommodate this change.\n\n"
        "I have forwarded your modification request to our team. "
        "They will confirm the details and update your order shortly."
    ),
    "limited": (
        "Thank you for letting us know. Your order is already in production, "
        "so changes may be limited.\n\n"
        "I have forwarded your request to our production manager who will "
        "assess the feasibility and get back to you within 2 hours."
    ),
    "blocked": (
        "I'm sorry, but your order has already been shipped and cannot be modified.\n\n"
        "If you need a new order with different specifications, please let me know "
        "and I will be happy to help!"
    ),
}

_ORDER_CHANGE_NOTIFY_TEAM = {
    "change_text": "sales_team",
    "change_material": "sales_team",
    "change_size": "production_team",
    "change_quantity": "sales_team",
    "change_shipping": "ops_manager",
    "cancel": "ops_manager",
}


def classify_order_change(text):
    """分类订单修改类型"""
    t = text.strip().lower()
    for change_type, keywords in _ORDER_CHANGE_KEYWORDS.items():
        if any(kw in t for kw in keywords):
            # 额外排除：cancel 要求更高的匹配精度
            if change_type == "cancel":
                if not any(kw in t for kw in ["cancel", "取消", "don't want", "不想要"]):
                    continue
            return change_type
    return None


def _try_order_change_flow(text, chat_name, messages):
    """检测并处理客户订单修改请求

    前置条件:
        - 客户有 active/in_production/shipped 的订单
        - 回复文本匹配修改关键词
    返回:
        True = 已处理，无需走 AI 分析
    """
    try:
        ...
        # 2. 分类修改类型
        change_type = classify_order_change(t)
        if not change_type:
            return False

        # 3. 查找可修改的订单
        from database import get_orders
        modifiable_statuses = ["active", "in_production", "shipped"]
        order = None
        ...
        _handle_order_change_request(c["id"], chat_name, text, change_type, order)
        return True

    except Exception as e:
        print(f"[OrderChange] 检测异常（放行）: {e}")
        return False


def _handle_order_change_request(cid, chat_name, text, change_type, order):
    """处理订单修改请求"""
    from database import add_timeline_entry, add_activity_log, get_active_user_ids, add_notification
    from datetime import datetime

    order_no = order.get("order_no", "")
    order_status = order.get("status", "")
    feasibility = _ORDER_CHANGE_FEASIBILITY.get(change_type, {}).get(order_status, "blocked")
    reply = _ORDER_CHANGE_REPLIES.get(feasibility, _ORDER_CHANGE_REPLIES["blocked"])
    notify_team = _ORDER_CHANGE_NOTIFY_TEAM.get(change_type, "sales_team")

    print(f"[OrderChange] {chat_name} -> {change_type} (order={order_no}, status={order_status}, feasibility={feasibility})")

    try:
        # 1. 记录到订单时间线
        add_timeline_entry(
            order["id"], order_status, 0,
            f"客户请求修改订单 [{change_type}]: {text[:120]} (可行性: {feasibility})"
        )
        ...
        if feasibility != "blocked":
            ...
            change_note = f"[{now}] ChangeRequest({change_type}): {text[:100]}"
            updated_notes = (existing_notes + "\n" + change_note).strip()
            from database import update_order
            update_order(order["id"], {"notes": updated_notes})

        # 3. 记录活动
        _add_wa_activity("order_change_request", cid, chat_name,
                         f"[{change_type}/{feasibility}] 订单{order_no}: {text[:60]}...")

        # 4. 通知责任人
        user_ids = get_active_user_ids()
        sla_note = ""
        if feasibility == "limited":
            sla_note = " (SLA: 2h)"
        elif feasibility == "blocked":
            sla_note = " (无需处理)"

        for auid in user_ids:
            add_notification(auid, 'order_change',
                f'🔄 Order Change Request — {chat_name}',
                f'[{change_type}/{feasibility}] 订单{order_no}: {text[:80]}{sla_note}',
                link=f'/orders/{order["id"]}',
                related_type='order', related_id=order["id"])

        # 5. 发送 WhatsApp 回复
        send_text(reply, contact_name=chat_name)

        print(f"[OrderChange] {chat_name} -> {change_type} ({feasibility}) ✅")

    except Exception as e:
        print(f"[OrderChange] 处理失败: {e}")
        import traceback
        traceback.print_exc()
        try:
            send_text(f"Thank you for your request. Our team will review and get back to you.",
                      contact_name=chat_name)
        except:
            pass


def _handle_confirm_design(cid, chat_name, quote):
    """A: 客户确认设计 —> 更新报价 —> 创建订单 —> 请求定金"""
    print(f"[ABC] {chat_name} 确认设计，quote#{quote['id']}")

    try:
        # 1. 更新报价状态：preview -> confirmed
        update_quote(quote["id"], {"status": "confirmed"})

        # 2. 更新客户状态
        update_lead_state(cid, "确认设计")
        set_lead_state(cid, "HOT")  # 确认设计 = 强购买信号

        # ====== V1.5: 自动创建订单 ======
        items = quote.get("items", [])
        total = quote.get("total_amount", 0)
        currency = quote.get("currency", "USD")
        deposit_amount = round(total * 0.3, 0)  # 30% 定金
        ref_no = quote.get("quote_no", "")
        ...
        order_data = {
            ...
            "status": "waiting_deposit",  # Financial Lock: 等待定金
            "notes": f"源自设计确认报价 {ref_no}",
            "created_by": 1,  # AI 系统
        }
        order_result = add_order(order_data)
        oid = order_result.get("id")
        order_no = order_result.get("order_no", "")

        # 报价标记为已转化
        update_quote(quote["id"], {"status": "converted"})

        add_timeline_entry(oid, "waiting_deposit", 0,
                           f"设计确认后自动创建，等待定金 (源自报价 {ref_no})")
        print(f"[V1.5] {chat_name} 订单已创建: {order_no} (ID={oid}), 定金={deposit_amount}")
        ...
        deposit_msg = (
            f"Your design has been confirmed and an order has been created.\n\n"
            f"*Order Summary*\n"
            f"Reference: {order_no}\n"
            f"Total Amount: {sym}{total:,.0f}\n"
            f"Deposit (30%): {sym}{deposit_amount:,.0f}\n"
            f"Balance Due: {sym}{balance_amount:,.0f}\n\n"
            f"To proceed with production, please arrange the deposit payment.\n"
            f"We accept T/T, Western Union, and PayPal.\n\n"
            f"Our bank details will be shared once you confirm.\n\n"
            f"Please reply:\n"
            f"*D* = Confirm deposit, send bank details \U0001f4b3\n"
            f"*E* = I have questions about the order ❓"
        )
        send_text(deposit_msg, contact_name=chat_name)
        print(f"[V1.5] {chat_name} 定金请求已发送")

        # ====== 记录活动 ======
        _add_wa_activity("design_confirmed", cid, chat_name,
                         f"客户确认设计，报价#{quote['id']} -> 订单{order_no}")
        _add_wa_activity("order_created", cid, chat_name,
                         f"自动创建订单 {order_no}，定金{sym}{deposit_amount:,.0f}")

        print(f"[ABC] {chat_name} 设计确认 -> 订单创建完成")

    except Exception as e:
        print(f"[ABC] 确认处理失败: {e}")
        import traceback
        traceback.print_exc()


def _handle_revise_design(cid, chat_name, text, quote):
    """B: 客户要求修改 —> 标记旧报价 + 重新设计"""
    print(f"[ABC] {chat_name} 要求修改设计，quote#{quote['id']}")

    try:
        # 1. 旧报价标记为 revised
        update_quote(quote["id"], {"status": "revised"})

        # 2. 提取修改要求（去掉 B 前缀）
        modify_text = text.strip()
        for prefix in ("b", "b.", "modify", "change", "revise"):
            if modify_text.lower().startswith(prefix):
                modify_text = modify_text[len(prefix):].strip().lstrip(".,:; ")
                break
        if not modify_text:
            modify_text = text  # 没提取到就用原文

        # 3. 从旧 quote 获取原始 schema
        original_schema = {}
        try:
            notes_raw = quote.get("notes", "")
            if "设计招牌自动生成:" in notes_raw:
                json_part = notes_raw.split("设计招牌自动生成:", 1)[1].strip()
                original_schema = json.loads(json_part)
        except Exception:
            pass

        # 4. 构造修改后需求 + 保存为收到的消息（DESIGN_SIGN 回调从 DB 读取）
        modified_request = f"Modify design: {modify_text}"
        if original_schema:
            modified_request += f". Original specs: {json.dumps(original_schema, ensure_ascii=False)}"
        add_message(cid, "received", "", modified_request)

        # 5. 先发一条收到修改的消息
        send_text("Got your modification! Working on the updated design...",
                  contact_name=chat_name)

        # 6. 重新执行 DESIGN_SIGN 回调（读取最新消息 = modified_request）
        execute_action("DESIGN_SIGN", cid, context={
            "chat_name": chat_name,
            "intent": "设计招牌",
            "analysis": {},
        })
        ...
        # 7. 记录活动
        _add_wa_activity("design_revised", cid, chat_name,
                         f"客户要求修改: {modify_text[:50]}")

        print(f"[ABC] {chat_name} 修改请求已提交，重新走 DESIGN_SIGN")

    except Exception as e:
        print(f"[ABC] 修改处理失败: {e}")


def _handle_reject_design(cid, chat_name, quote):
    """C: 客户拒绝 —> 标记 + 人工跟进"""
    print(f"[ABC] {chat_name} 拒绝设计，quote#{quote['id']}")

    try:
        # 1. 报价标记为 rejected
        update_quote(quote["id"], {"status": "rejected"})

        # 2. 更新客户状态
        update_lead_state(cid, "设计拒绝")

        # 3. 发送回退消息
        msg = (
            f"I'm sorry the design didn't meet your expectations.\n\n"
            f"Could you let me know what you're looking for? We have a wide range of "
            f"options — different materials, sizes, and lighting effects.\n\n"
            f"Alternatively, one of our senior designers can contact you directly "
            f"to discuss your requirements in detail."
        )
        send_text(msg, contact_name=chat_name)

        # 4. 记录活动
        _add_wa_activity("design_rejected", cid, chat_name,
                         f"客户拒绝设计，需人工跟进")

        print(f"[ABC] {chat_name} 设计拒绝，已标记")

    except Exception as e:
        print(f"[ABC] 拒绝处理失败: {e}")
...
def _whatsapp_message_handler(chat_name, messages):
    ...
    try:
        ...
        # ====== V1.2 A/B/C 确认回复检测（快速路径，AI 分析前）======
        if _try_abc_flow(text, chat_name, messages):
            return

        # ====== V1.6 D/E 资金启动 & 人工路由（快速路径，AI 分析前）======
        if _try_deposit_flow(text, chat_name, messages):
            return

        # ====== V1.8 Payment Receipt Gate: 客户确认已付款 ======
        if _try_receipt_flow(text, chat_name, messages):
            return

        # ====== V1.9 Order Status Inquiry: 客户查询订单状态 ======
        if _try_order_status_flow(text, chat_name, messages):
            return

        # ====== Order Change Request: 客户要求修改/取消订单 ======
        if _try_order_change_flow(text, chat_name, messages):
            return

        # ====== Transaction Intent Blacklist: 交易关键词强制不走 AI ======
        _TRANSACTION_KEYWORDS = [
            "bank account", "account number", "swift", "bank transfer",
            "remittance", "wire transfer", "tt", "t/t", "routing number",
            "iban", "beneficiary", "payee",
        ]
        t_lower = text.strip().lower()
        if any(kw in t_lower for kw in _TRANSACTION_KEYWORDS):
            # 重试 D flow（关键词匹配更宽松的第二轮）
            print(f"[Blacklist] {chat_name} 命中交易关键词，绕过 AI")
            from database import get_orders
            _all_custs = get_customers()
            _bc = next((_c for _c in _all_custs if _c["name"] == chat_name), None)
            if _bc:
                _pending_orders = get_orders(status="waiting_deposit", customer_id=_bc["id"])
                if _pending_orders:
                    _handle_deposit_request(_bc["id"], chat_name, _pending_orders[0])
                    return
        ...
        if not c:
            ...
            result = add_customer(name=chat_name, status="new", country=wa_country, notes="WhatsApp自动添加")
        ...
        # 先保存接收到的消息（即使 AI 分析失败也不丢数据）
        add_message(c["id"], "received", "", text)
        ...
        try:
            ...
            # 保底：告诉客户有人工跟进，不让客户干等
            _send_fallback_reply(c["id"], chat_name, text, customer_country)
            return
        ...
        # ====== Agent Coordinator: 获取每客户锁（intent 已知后才选择主导 Agent） ======
        _coord_lock_held = False
        _lead_agent = "ai_assistant"
        try:
            from safety.agent_coordinator import AgentCoordinator
            _coord = AgentCoordinator()
            _lead_state = c.get("lead_state", "NEW")
            _lead_agent = _coord.select_lead_agent(c["id"], intent, _lead_state)
            _coord_lock_held = _coord.lock(c["id"], _lead_agent)
            if _coord_lock_held:
                print(f"[Coordinator] {chat_name} → 主导 Agent: {_lead_agent}")
        except ImportError:
            pass
        except Exception as e:
            print(f"[Coordinator] 异常（放行）: {e}")
        ...
        if decision["action"] != "ROUTINE_REPLY":
            ...
            # DESIGN_SIGN 回调已发送概念图+文字，阻止 _do_auto_reply 重复发送
            if decision["action"] == "DESIGN_SIGN":
                reply_en = ""
        ...
        # ====== Execution Firewall: AI 动作拦截 ======
        _fw_bypass = True   # 默认直接发送
        _fw_verdict = None
        try:
            from safety.execution_firewall import ExecutionFirewall
            from safety.state_registry import StateRegistry
            ...
            _fw_action = {
                ...
                "content": reply_en,
                "intent": intent,
                "source_agent": _lead_agent if _coord_lock_held else "ai_assistant",
            }
            _fw_ctx = {
                "customer_country": customer_country,
                "customer_tier": c.get("tier") or c.get("lead_status", "LOW"),
                "urgency": analysis.get("urgency", "中"),
                "translation": translation,
            }
            _fw_decision = _fw.check(c["id"], _fw_action, _fw_ctx)
            _fw_verdict = _fw_decision["verdict"]
            if _fw_verdict == "BLOCK":
                print(f"[Firewall] ⛔ {chat_name} 拦截: {_fw_decision['reason']}")
                _add_wa_activity("blocked", c["id"], chat_name, f"防火墙拦截: {_fw_decision['reason']}")
                reply_en = ""  # 清空回复，跳过发送
            elif _fw_verdict == "MODIFY" and _fw_decision.get("modified_action"):
                reply_en = _fw_decision["modified_action"].get("content", reply_en)
                print(f"[Firewall] ✏️ {chat_name} 修正后放行: {_fw_decision['reason']}")
            elif _fw_verdict == "ESCALATE":
                _fw_bypass = False  # 高风险 → 走审核队列
                print(f"[Firewall] ⚠ {chat_name} 需人工审核: {_fw_decision['reason']}")
                _add_wa_activity("escalated", c["id"], chat_name, f"需审核: {_fw_decision['reason']}")
            elif _fw_verdict == "FLAG":
                print(f"[Firewall] ⚠ {chat_name} 标记待审查: {_fw_decision['reason']}")
        except ImportError:
            pass  # 无防火墙模块 → 直接放行
        except Exception as e:
            print(f"[Firewall] 异常（放行）: {e}")
        ...
        if reply_en and _auto_reply_enabled:
            ...
            t = threading.Thread(
                ...
                kwargs={"bypass_queue": _fw_bypass},
                daemon=True
            )
            t.start()
        else:
        ...
        # ====== Agent Coordinator: 释放锁 ======
        if _coord_lock_held:
            try:
                _coord.unlock(c["id"])
                print(f"[Coordinator] {chat_name} 锁已释放")
            except Exception:
                pass
        ...
        print(f"[Auto] 处理出错: {e}")
        try:
            _send_fallback_reply(c["id"], chat_name, text if 'text' in dir() else "", customer_country if 'customer_country' in dir() else "")
        except Exception:
            pass
...
def api_update_order(oid):
    ...
    if "status" in data and existing:
        if existing.get("status") != data["status"]:
            ...
            if new_status in ("active", "confirmed", "in_production", "shipped", "delivered"):
    ...
    # ====== Revenue Closure Layer: 交货完成 → 评估财务终态 ======
    if data.get("status") == "delivered" and existing and existing.get("status") != "delivered":
        try:
            refreshed = get_order(oid)
            if refreshed:
                revenue_status = evaluate_revenue_closure(refreshed)
                update_order(oid, {"revenue_status": revenue_status})
                add_timeline_entry(oid, revenue_status, session.get("user_id", 0),
                                   f"Revenue closure: {revenue_status}")
                print(f"[RevenueClosure] 订单 {refreshed.get('order_no','')} "
                      f"已交货 → revenue_status={revenue_status}")
        except Exception as e:
            print(f"[RevenueClosure] 评估异常（放行）: {e}")
...
def api_order_payment(oid):
    ...
    # ====== Financial Lock: 定金到账 → 订单激活 ======
    if ptype == "deposit" and order and order.get("status") == "waiting_deposit":
        update_order(oid, {"status": "active"})
        # 更新 deposit_status（如果客户已确认付款）
        if order.get("deposit_status") == "payment_confirmed":
            update_order(oid, {"deposit_status": "payment_received"})
        add_timeline_entry(oid, "active", session.get("user_id", 0),
                           "定金已到账，订单已激活")
        # Notify customer that order is active
        customer = get_customer(order.get("customer_id"))
        if customer and customer.get("whatsapp"):
            def _activate_job():
                _notify_order_status_change(order, customer, "waiting_deposit", "active")
            threading.Thread(target=_activate_job, daemon=True).start()
        print(f"[FinancialLock] 订单 {order.get('order_no','')} 定金到账 → active")

    # ====== Revenue Closure Layer: 付款到账 → 重新评估财务终态 ======
    if order and order.get("status") == "delivered":
        try:
            refreshed = get_order(oid)
            if refreshed:
                revenue_status = evaluate_revenue_closure(refreshed)
                if revenue_status != refreshed.get("revenue_status"):
                    update_order(oid, {"revenue_status": revenue_status})
                    add_timeline_entry(oid, revenue_status, session.get("user_id", 0),
                                       f"Revenue closure: {revenue_status}")
                    print(f"[RevenueClosure] 订单 {refreshed.get('order_no','')} "
                          f"收到付款 → revenue_status={revenue_status}")
        except Exception as e:
            print(f"[RevenueClosure] 评估异常（放行）: {e}")
...
def _init_lead_state_engine():
    ...
    def _callback_design_sign(cid, ctx):
        """设计招牌：抽取参数 → 出概念图 → 估算报价 → WhatsApp 发送"""
        chat_name = ctx.get("chat_name", "")
        print(f"[Action] DESIGN_SIGN 客户#{cid} {chat_name}")
        ...
        try:
            ...
            for m in msgs:
                if m["direction"] == "received":
                    ...
                    break
            if not latest_text:
                latest_text = chat_name

            # 2. 提取结构化招牌参数
            customer_country = c.get("country", "")
            schema = extract_sign_schema(latest_text, country=customer_country)
            print(f"[DesignSign] Schema: {json.dumps(schema, ensure_ascii=False)}")

            # 3. 生成概念图
            parsed, image_url, img_err = generate_sign_prompt_and_image(
                sign_text=schema["text"],
                material=schema["material"],
                scene=schema.get("background", "night"),
            )
            if img_err or not image_url:
                print(f"[DesignSign] 出图失败: {img_err}")
                image_url = None

            # 4. 估算价格
            price_min, price_max, currency = _estimate_sign_price(schema)
            print(f"[DesignSign] 估价: {currency} {price_min}–{price_max}")

            # === V1 Stabilization Patch: 一致性锁 ===
            from ai_engine import sign_schema_hash
            schema_hash = sign_schema_hash(schema)
            print(f"[DesignSign] Schema Hash: {schema_hash}")

            # 5. 保存报价草稿
            from datetime import date, timedelta
            ref_no = f"DS-{datetime.now().strftime('%Y%m%d')}-{chat_name[:4].upper()}"
            schema_copy = dict(schema)
            quote_data = {
                "customer_id": cid,
                "quote_no": ref_no,
                "currency": currency,
                "total_amount": price_max,
                "valid_until": (date.today() + timedelta(days=15)).isoformat(),
                "status": "preview",
                "schema_hash": schema_hash,
                "notes": f"设计招牌自动生成: {json.dumps(schema_copy, ensure_ascii=False)}",
                "created_by": 1,
                "items": [{
                    "product_id": "",
                    "name": f"LED {schema['text']} ({schema['material']}, {schema['letter_height_cm']}cm)",
                    "qty": schema["quantity"],
                    "unit": "set",
                    "unit_price": price_max,
                    "total": price_max * schema["quantity"],
                }],
            }
            db_result = add_quote(quote_data)
            qid = db_result.get("id")

            # 6. 保存到 ai_generations 表
            try:
                add_ai_generation("design_sign", json.dumps(schema_copy, ensure_ascii=False),
                                  image_url or "", customer_id=cid,
                                  metadata=json.dumps({"schema_hash": schema_hash}))
            except Exception:
                pass

            # 7. 发送 WhatsApp 消息（图片 + 规格 + 报价 + A/B/C）
            _send_design_sign_whatsapp(chat_name, schema, image_url, price_min, price_max, currency, ref_no, schema_hash)

            detail = (f"客户#{cid} {chat_name}: 设计招牌 {schema['text']} "
                      f"({schema['material']}, {schema['letter_height_cm']}cm), "
                      f"估价 {currency} {price_min}–{price_max}")
            ...
            print(f"[Action] ❌ DESIGN_SIGN 失败: {e}")
            import traceback
            traceback.print_exc()
            return {"ok": False, "error": str(e)}

    def _estimate_sign_price(schema):
        """根据招牌参数估算价格区间"""
        currency = "USD"
        MATERIAL_RATES = {
            "ss304": 3.5, "ss316": 4.5, "ss304_gold": 4.0, "ss304_rosegold": 4.0,
            "ss304_black": 3.8, "ss304_blue": 3.8, "ss304_purple": 3.8,
            "zn": 2.5, "zn_paint": 2.8, "acrylic": 2.0, "aluminum": 3.0,
        }
        height = schema.get("letter_height_cm", 50)
        material = schema.get("material", "ss304")
        qty = schema.get("quantity", 1)
        rate = MATERIAL_RATES.get(material, 3.0)
        base = height * rate * qty
        kelvin = schema.get("kelvin", "")
        if "RGB" in kelvin:
            base *= 1.4
        elif "Warm White" in kelvin:
            base *= 1.1
        ptype = schema.get("product_type", "front")
        if ptype == "double":
            base *= 1.6
        elif ptype == "neon":
            base *= 1.3
            ...
            base *= 1.5
        base += 30 * qty  # 木箱包装
        price_min = round(base * 0.9, -1)
        price_max = round(base * 1.15, -1)
        return max(price_min, 50), max(price_max, 80), currency

    def _send_design_sign_whatsapp(chat_name, schema, image_url, price_min, price_max, currency, ref_no, schema_hash=""):
        """发送设计招牌结果到 WhatsApp"""
        sym = "$" if currency == "USD" else "¥"
        ...
        if image_url:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    tmp_path = os.path.join(tmp_dir, f"design_{chat_name}_{int(time.time())}.jpg")
                    with open(tmp_path, "wb") as f:
                        f.write(resp.content)
                    send_image_clipboard(tmp_path, contact_name=chat_name)
                    print(f"[DesignSign] 已发送概念图给 {chat_name}")
                else:
                    print(f"[DesignSign] 图片下载失败: {resp.status_code}")
            except Exception as e:
                print(f"[DesignSign] 发送图片异常: {e}")

        # 2. 发送规格 + 报价文字
        material_labels = {
            "ss304": "304 Stainless Steel", "ss316": "316 Stainless Steel",
            "ss304_gold": "304 Gold Finish", "ss304_rosegold": "304 Rose Gold",
            "ss304_black": "304 Black", "ss304_blue": "304 Blue", "ss304_purple": "304 Purple",
            "zn": "Zinc Letters", "zn_paint": "Painted Zinc",
            "acrylic": "Acrylic", "aluminum": "Aluminum",
        }
        mat_label = material_labels.get(schema.get("material", ""), schema.get("material", "Stainless Steel"))

        spec_lines = [
            f"🎨 *Design Concept for Your Sign*",
            f"",
            f"📋 Reference: {ref_no}",
            f"",
            f"*Specifications:*",
            f"• Text: {schema.get('text', '')}",
            f"• Material: {mat_label}",
            f"• Letter Height: {schema.get('letter_height_cm', '?')}cm",
            f"• Thickness: {schema.get('letter_thickness_cm', '?')}cm",
            f"• Lighting: {schema.get('kelvin', 'Cool White')}",
            f"• Quantity: {schema.get('quantity', 1)}",
            f"",
            f"💰 *Estimated Price: {sym}{price_min} – {sym}{price_max}*",
            f"",
            f"_Ref: {ref_no} | Design ID: {schema_hash}_",
            f"",
            f"_Please reply:_",
            f"*A* = Confirm, prepare formal quote ✅",
            f"*B* = Modify design ✏️",
            f"*C* = Not what I need ❌",
        ]
        msg = "\n".join(spec_lines)
        ...
        send_text(msg, contact_name=chat_name)
        print(f"[DesignSign] 已发送规格报价给 {chat_name}")
    ...
    def _callback_generate_quote(cid, ctx):
        ...
        try:
            ...
            # 2. Agent Coordinator: 冻结价格快照
            try:
                from safety.agent_coordinator import AgentCoordinator
                _coord = AgentCoordinator()
                _price_oracle = _coord.get_price_oracle(cid)
                if _price_oracle:
                    print(f"[Coordinator] 价格快照已冻结: {len(_price_oracle)} 锚点")
            except ImportError:
                pass
            except Exception as e:
                print(f"[Coordinator] 价格快照异常: {e}")
    ...
    ar.register_action("DESIGN_SIGN", _callback_design_sign)
    ar.register_action("FOLLOW_UP", _callback_follow_up)
    ar.register_action("ESCALATE", _callback_escalate)
    print(f"[LeadState] 动作回调已注册: GENERATE_QUOTE, SEND_QUOTE, DESIGN_SIGN, FOLLOW_UP, ESCALATE")
...
# ==================== UEE Control Plane API ====================

@app.route("/api/control-plane/escalations")
@login_required
def api_uee_list():
    """GET /api/control-plane/escalations?type=PAYMENT&status=open&limit=50
    返回 UEE 统一队列
    """
    try:
        from database import get_unified_escalations
        etype = request.args.get("type")
        status = request.args.get("status", "open")
        limit = int(request.args.get("limit", 50))
        events = get_unified_escalations(
...
@app.route("/api/control-plane/escalations/sla-risk")
@login_required
def api_uee_sla_risk():
    """GET /api/control-plane/escalations/sla-risk?threshold=60&limit=20
    返回 SLA 高风险列表
    """
    try:
        from database import get_sla_risk_escalations
        threshold = int(request.args.get("threshold", 60))
        limit = int(request.args.get("limit", 20))
        events = get_sla_risk_escalations(
...
@app.route("/api/control-plane/escalations/resolve", methods=["POST"])
@login_required
def api_uee_action():
    """POST /api/control-plane/escalations/resolve
    {
        "id": 123,
        "action": "resolve | escalate | assign | dismiss",
        "owner": "user_001",
        "note": "Payment confirmed, closing escalation"
    }
    """
    try:
        data = request.json or {}
        ...
        owner = data.get("owner", session.get("user_id", "unknown"))
        note = data.get("note", "")

        from database import action_unified_escalation
        ok = action_unified_escalation(eid, action_type, str(owner), note)
        if ok:
            return jsonify({"ok": True, "message": f"Escalation #{eid} -> {action_type}"})
        else:
        ...
        while True:
            try:
                ...
                for entry in expired:
                    reply_text = entry.get("edited_en") or entry["reply_en"]
                    # V0-SAFETY: content gate before send
                    try:
                        from safety.content_gate import validate_outbound_message
                        from safety.message_audit import log_outbound
                        gate_result = validate_outbound_message(
                            entry["customer_id"], reply_text,
                            {"tier": "LOW"},
                        )
                        if not gate_result["passed"]:
                            print(f"[ReviewQueue] ⛔ 内容审核拦截: {entry['id']} -> {entry['customer_name']}")
                            # Mark as blocked — don't send
                            queue_data = review_queue._load()
                            for qe in queue_data:
                                if qe["id"] == entry["id"] and qe["status"] == "pending":
                                    qe["status"] = "blocked"
                                    qe["blocked_rules"] = gate_result["blocked_rules"]
                                    import json as _json
                                    from datetime import datetime as _dt
                                    qe["reviewed_at"] = _dt.now().isoformat()
                                    break
                            review_queue._save(queue_data)
                            log_outbound(entry["customer_id"], reply_text,
                    ...
                    send_text(reply_text, contact_name=entry["customer_name"])
                    from database import add_message
                    add_message(entry["customer_id"], "sent",
